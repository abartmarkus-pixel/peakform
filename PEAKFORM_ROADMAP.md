...
### ✅ Kilometer-Splits im Claude-Analyse-Prompt nutzten elapsed_time statt moving_time (behoben 26.7.2026)
Anlass: Nutzer bemerkte, dass der Analysetext für km 1 eines Laufs „7:48" nannte, während die Kilometer-Splits-Tabelle für denselben Kilometer „7:24" zeigte — beide Werte stammen aus demselben `splits_metric_json`. Ursache: Der Kilometer-Splits-Abschnitt des Prompts in `analyzeActivity()` (`src/lib/activityAnalysis.ts`) las `elapsed_time` (inkl. Pausen, z. B. GPS-Lock am Start) statt `moving_time`, das die UI-Tabelle in `ActivityDetail.tsx` bereits seit einem früheren Fix verwendet. Behoben durch Angleichung an `moving_time`.

### ✅ Aktivitätsdauer ab 60 Minuten in Std+Min statt roher Minutenzahl (behoben 26.7.2026)
Anlass: Wochenreview nannte eine über 6 Stunden lange Aktivität als „386 Minuten" statt verständlicher „6 Std 26 Min". Betraf neben dem Wochenreview-Prompt (`WeeklyPlan.tsx`) auch den Aktivitäts-Analyse-Prompt (`activityAnalysis.ts`), den Echtzeit-Alert-Konfliktcheck-Prompt (`Dashboard.tsx`) sowie zwei UI-Textstellen in `WeeklyPlan.tsx` — alle nutzten dieselbe rohe `Math.round(duration_s / 60)`-Berechnung. Neuer gemeinsamer Helper `formatDurationHuman(seconds)` (`src/lib/dateUtils.ts`) an allen Stellen eingesetzt.

### ✅ ICS-Kalender-Feed für Wochenplan — Apple Kalender abonnierbar (implementiert 27.7.2026)
Wunsch: geplante Einheiten automatisch im Apple Kalender sehen, Kraft-Einheiten mit 💪🍑-Präfix, Laufen mit 🏃, Radfahren mit 🚴. Da PeakForm eine PWA ohne native App ist, keine direkte EventKit-Anbindung möglich — stattdessen neuer öffentlicher ICS-Feed `api/calendar/[athleteId].ts` (GET, kein Session-Auth; `athleteId`-UUID in der URL dient als Capability-Token, analog zur bereits offenen RLS-Policy). Reine Generierungslogik in `api/_lib/ics.ts` (`buildIcsFeed`), von der Vercel-Function UND einer neuen lokalen Dev-Middleware (`vite.config.ts`) genutzt — dedupliziert `weekly_plans` auf die neueste `version` je `week_start`, lässt Ruhetage aus, stabile UID pro Tag (kein Duplikat bei wiederholtem Abo-Polling). Titel-Schema: Kraft-`description` ist immer kurz ("Workout I/II/III") und wandert direkt in den Titel; Lauf/Rad-`description` ist dagegen freier Coaching-Fließtext — der bleibt im `SUMMARY` unlesbar lang, deshalb kurzer Titel (`🏃 Laufen · 35 Min`) + Volltext in `DESCRIPTION`. Abo-Link-Button neu in `Profile.tsx` zwischen Krafttraining-Sektion und Konto löschen.

Nach echtem Live-Test auf dem iPhone (28.7.2026) drei Nachbesserungen:
- **Routing-Bug (kein Feed erreichbar):** der SPA-Catch-all-Rewrite in `vercel.json` (`"/(.*)" -> /index.html`) fing jeden `/api/calendar/*`-Request ab, bevor er die Funktion erreichte — bestätigt über Vercel Runtime-Logs (0 Invocations für die Route, während `/api/analyse`/`/api/strava-token` normal liefen). Fix: Rewrite schließt `/api/` per Negative-Lookahead aus.
- **Funktion crashte danach mit `ERR_MODULE_NOT_FOUND`:** Vercels Node-Runtime führt kompilierte API-Funktionen als natives ESM aus, das anders als `tsc` mit `moduleResolution: bundler` keine Extension-Auflösung macht — der Import `../_lib/ics` (ohne `.js`) schlug zur Laufzeit fehl, obwohl `tsc` lokal fehlerfrei durchlief. Fix: `.js`-Extension im Import ergänzt.
- **All-Day-Events + `webcal://` verhinderten sinnvolle Nutzung in iCloud-Kalender:** Termine liefen jetzt zwar durch, aber ganztägig statt zur geplanten Zeit, und der `webcal://`-Link wurde von iOS' manuellem „Kalenderabo hinzufügen"-Dialog beim Einfügen zu `http://` umgewandelt (Vercel redirected http→https, was die Verifizierung zusätzlich erschwerte). Fix: Events starten jetzt einheitlich um 18:30 Uhr Europe/Vienna (DST-sicher über dieselbe Intl-longOffset-Technik wie in `api/send-daily-reminder.ts`) für die im Plan hinterlegte `duration_min` (Default 60 Min), inkl. `CATEGORIES:Sport` pro VEVENT für Apple Kalenders Tag-Farbzuordnung; Abo-Link-Button kopiert jetzt `https://` statt `webcal://`.
- **Feed zeigte auch alle Vergangenheits-Wochen (behoben 27.7.2026):** `buildIcsFeed()` gab bisher jede `weekly_plans`-Woche aus, egal wie lange sie zurücklag. Fix: neue Hilfsfunktion `currentWeekStartVienna()` (Montag der laufenden Woche in Europe/Vienna, analog `viennaTodayInfo()` in `api/send-daily-reminder.ts`) filtert Wochen vor der aktuellen Woche vor dem Event-Aufbau heraus — der Feed enthält jetzt nur noch die laufende und zukünftige Wochen.
...
### ✅ Grafik zur Entwicklung der geschätzten Zielzeit (implementiert 19.8.2026)
Die Zielzeit-Schätzung auf der Ziel-Detailseite (siehe oben) war bisher nur eine Momentaufnahme — kein Hinweis darauf, ob sich die Form verbessert. `calculateFinishTimeProgression()` (`coachContext.ts`) rechnet dieselbe Methodenkette (Effizienz-Modell → Distanz-Anker-Fallback) rückwirkend an 8 Zeitpunkten im 14-Tage-Abstand neu (letzte 98 Tage) — jeweils nur mit den Läufen/Strava-PRs, die zum jeweiligen Zeitpunkt bereits vorlagen, damit kein späterer Formfortschritt fälschlich in frühere Punkte durchsickert. `GoalDetail.tsx` zeigt das als Recharts-Linienchart mit invertierter Y-Achse (oben = schneller).

### ✅ Saisonziel-Woche und Review-Einschränkungen hart in der Wochenplan-Generierung abgesichert (behoben 13.9.2026)
Zwei reale Fehlplanungen: (1) Ein Saisonziel konnte trotz korrekt genanntem Datum in der falschen Woche als Renntag landen, weil Claude Wochentag↔Datum selbst verrechnen musste (Vorfall: „Tiroler Firmenlauf" eine Woche zu früh eingetragen). Fix: `checkGoalPlacement()` prüft nach der Generierung deterministisch anhand von `event_date`, ob/an welchem Wochentag ein Ziel in der aktuellen Planwoche steht, plus ein fest berechneter Prompt-Abschnitt, der Claude die Wochentag-Zuordnung von vornherein abnimmt. (2) Im Wochenreview eingetragene körperliche Einschränkungen flossen nur unpriorisiert im Freitext ein und konnten bei der nächsten Plan-Generierung untergehen. Fix: Claude liefert beim Review jetzt explizit ein `constraint`-Feld; gesetzt landet es als eigener `coach_decisions`-Eintrag in derselben Höchste-Priorität-Regel wie automatisch erkannte Verletzungssignale.

### ✅ Intervallstruktur-Erkennung in der Aktivitäts-Analyse ohne brauchbare Rundendaten (implementiert 16.9.2026, Lauf + Rad)
Fehlen Runden oder sind die vorhandenen nur generische geräteseitige Auto-Splits (feste 1km-/Zeit-Laps statt echter Intervall-Runden), UND der Plantag gibt Z4/Z5 vor oder die Aktivität ist manuell als Intervalltraining markiert (neuer Toggle „Intervalltraining?" in `ActivityDetail.tsx`), segmentiert `detectEffortSegments()` den ohnehin vorhandenen Sekundenverlauf (Tempo/Watt + Puls) deterministisch in Belastungs-/Erholungsblöcke — Schwelle und Mindest-Intervalldauer ergeben sich aus der Aktivität selbst statt aus festen Zonen-Grenzen. Ersetzt den ursprünglichen Ansatz, Claude eine rohe Sekundentakt-Liste zum Selbst-Erkennen zu geben — der war in der Praxis zu ungenau (ein echtes 6×1km-Schwellentraining wurde daraus als 5 Blöcke à 7-15 Min statt der tatsächlichen 6 Wiederholungen gelesen). Beim Testen zusätzlich aufgefallen und mitbehoben: der Coach nannte Empfehlungen für einen anderen Plantag als heute fälschlich „heute" — neue Prompt-Regel verlangt jetzt die explizite Wochentag-Nennung.

### ✅ Sportspezifische Periodisierung Rad/Lauf nach Trainingsphilosophie (implementiert 16.9.2026)
Anlass: Vorbereitung auf ein Rad-Event ab Ende Oktober mit Ziel im April (~24-26 Wochen Vorlauf) — der Plan sollte nach validierten sportwissenschaftlichen Prinzipien statt rein freiem Claude-Ermessen laufen. Zwei Lücken geschlossen: (1) `calculateSeasonPhase()` nutzte für jedes A-Ziel dieselben, ursprünglich rein für Laufen geschriebenen Phasentexte ("Sehnen, Gelenke und Laufmuskulatur readaptieren" o.ä.) — bekommt jetzt einen optionalen `sportType`-Parameter, der bei einem Rad-Ziel passende Formulierungen liefert (Grundlage/Aufbau/Wettkampfvorbereitung/Taper). (2) Die eigentliche Zonen-/Intervallverteilung pro Woche war komplett Claudes freiem Ermessen überlassen, ohne kodierte Periodisierungslogik. Neue harte Regel „SPORTSPEZIFISCHE PERIODISIERUNG" in `generatePlan()` (nur wenn Rad oder Laufen das primäre A-Ziel ist) gibt phasenabhängig den passenden Intervalltyp vor (Coggan/Allen-Zonen fürs Rad, Daniels/Pfitzinger fürs Laufen, Seiler-Polarisierung für den 80/20-Zweig), bewusst ohne starre Prozent-Zonen-Vorgabe. Neues optionales Profil-Feld „Ø Std./Woche" pro Sportart steuert zusätzlich `determineTrainingPhilosophy()`: Polarisiert 80/20 ab 8h/Woche (Rad) bzw. 5h/Woche (Lauf), darunter ein effizienzfokussierter Ansatz (Sweet-Spot-Base/Schwellen-fokussiert). Im selben Zug die bisher falsch fixe Taper-Regel ("-30% Woche 13, -50% Woche 14", unpassend für Blöcke ungleich ~14 Wochen) auf eine relative Formulierung zum echten Event-Datum umgestellt.
...
| ✅ Zielzeit-Progressions-Grafik | Umgesetzt | — | 19.8.2026 |
