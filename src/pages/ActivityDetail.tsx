...
import { fetchActivityStreams, fetchActivityLaps, fetchActivityDetail, saveStravaPrsIfPresent, getValidAccessToken, type StravaLap, type StravaSplitMetric } from '../lib/strava'
import {
  analyzeActivity,
...
// Hevy-Übungsnamen sind je nach Nutzer-Locale Englisch ODER Deutsch (z.B. "Fliegende
// (Kurzhantel)" statt "Chest Fly") — beide Sprachen brauchen eigene Keywords, da
// .includes() rein textbasiert matcht und "fliegende" kein Substring von "fly" ist.
// Deutsche Einträge stehen nur dort separat, wo sie ein ANDERES Label als der
// generische englische Fallback derselben Wortfamilie hätten (z.B. "enges
// Bankdrücken" → Trizeps, nicht das generische Brust von "Bankdrücken") — sonst
// reicht ein einziges deutsches Catch-all-Keyword pro Familie, weil alle
// spezifischeren Varianten es ohnehin als Substring enthalten.
...
export default function ActivityDetail() {
            ...
            ? act.splits_metric_json
              ...
              : fetchActivityDetail(token, Number(id))
                  .then(async (d) => {
                    ...
                    await saveStravaPrsIfPresent(athlete.id, d.best_efforts, act.date)
                    return sm
                  })
  ...
  async function loadRecoWeek(which: 'current' | 'next', sport: PlanSport, preferDay?: string | null) {
    ...
    const result = await loadMatchingDays(athleteId, weekStart, sport)
    if (!result) {
      setRecoDayOptions([])
      setRecoDay('')
      return
  ...
  async function openRecoModal(sport: PlanSport) {
    ...
    setRecoDraft(null)
    try {
      const draft = await extractPlanRecommendation({ analysisText: analysis, sport })
      setRecoDraft(draft)
      ...
      await loadRecoWeek(which, sport, draft.day)
    } catch (e) {
      console.error(e)
  ...
  async function confirmApplyRecommendation(sport: PlanSport) {
    if (!activity || !athleteId || !recoDay) return
    setRecoSaving(true)
    setRecoError(null)
    try {
  ...
  // Nur Laufen/Radfahren: Kraft-Tage tragen einen festen "Workout I/II/III"-Namen
  // (siehe CLAUDE.md), eine Freitext-Empfehlung passt dort nicht in die description.
  const activitySportForPlan = activity ? sportFromActivityType(activity.type) : null
  const recoSport: PlanSport | null = activitySportForPlan === 'running' || activitySportForPlan === 'cycling' ? activitySportForPlan : null
  ...
  return (
    <>
      ...
      <div className="mb-6 flex justify-between gap-3">
        ...
        {analysis && recoSport && (
          <button
            onClick={() => openRecoModal(recoSport)}
            className="flex items-center justify-center gap-2 px-4 py-3 rounded-xl text-base font-semibold text-slate-200 bg-slate-800 hover:bg-slate-700 transition-colors"
          >
            ...
            {recoDraft && !recoLoading && (
              <>
                ...
                <div className="flex gap-3 pt-1">
                  ...
                  <button
                    onClick={() => confirmApplyRecommendation(recoSport)}
