...
> Letzte Aktualisierung: 20. September 2026 (Bugfix Wochenplan-Tages-Keys mit Datum — `normalizePlanDayKeys()`; lokaler Dev-Login + API-Weiterleitung, siehe Kapitel 10 und 14; davor 16. September 2026: Bugfix: Muskelgruppe-Pill in Übungskarten (WeightTraining) fehlte bei deutsch benannten Hevy-Übungen — `MUSCLE_LABELS`/`primaryMuscleLabel()` (`ActivityDetail.tsx`) enthielt bisher nur englische Keywords; deutsche Übungsnamen wie „Fliegende", „Vorgebeugtes Rudern", „Seitheben", „Einbeiniges Beckenheben", „Trizepsdrücken", „Clamshell" und „Pallof Press" matchten dadurch keinen Eintrag und zeigten keine Pille, obwohl andere Übungen mit englischem Namen (z. B. „Chin Up", „Bulgarian Split Squat", „Hammer Curl") korrekt funktionierten — Hevy überträgt selbst keine Muskelgruppen-Metadaten, die Pille wird komplett aus dem Übungsnamen geraten. Fix: deutsche Keyword-Äquivalente je Wortfamilie ergänzt (Bankdrücken/Fliegende/Klimmzug/Latzug/Rudern/Schulterdrücken/Seitheben/Frontheben/Schulterheben/Kniebeuge/Beinpresse/Ausfallschritt/Beinstrecker/Beinbeuger/Kreuzheben/Hüftheben/Beckenheben/Gesäßbrücke/Wadenheben/Trizepsdrücken sowie „Pallof Press" und „Clamshell") — spezifische Begriffe mit abweichendem Label (z. B. „enges Bankdrücken" → Trizeps) stehen vor dem generischen Fallback derselben Familie (z. B. „Bankdrücken" → Brust), analog zur bestehenden Reihenfolge bei den englischen Keywords; siehe Kapitel 9 „ActivityDetail.tsx"; davor Feature: Sportspezifische Periodisierung (Rad/Lauf) nach Trainingsphilosophie in der Wochenplan-Generierung — Anlass: Athlet bereitet sich ab Ende Oktober auf ein Rad-Event im April vor, wollte einen Plan nach validierten sportwissenschaftlichen Prinzipien statt rein freiem Claude-Ermessen. `calculateSeasonPhase(weeksUntilEvent, override, sportType?)` (`coachContext.ts`) bekommt einen dritten, optionalen Parameter `sportType` (`season_goals.sport_type` des A-Ziels) — bei `"Radfahren"` liefert eine neue `PHASE_LABELS_CYCLING`-Map die Phasentexte (Phase 1 „Grundlage" statt „Readaptation/Sehnen-Laufmuskulatur", Phase 2 „Aufbau" statt „Grundlagenaufbau/Kilometerzahl", Phase 3/4 mit Rad-Wortlaut) statt der ursprünglich rein für Laufen geschriebenen `PHASE_LABELS` — alle 6 Aufrufstellen (`coachPrompt.ts`, `WeeklyPlan.tsx`, `GoalDetail.tsx` ×2, `Profile.tsx` ×2) geben jetzt den Sporttyp mit. Neue harte Regel „SPORTSPEZIFISCHE PERIODISIERUNG" in `generatePlan()` (`WeeklyPlan.tsx`) greift NUR, wenn das primäre A-Ziel „Radfahren" oder „Laufen" ist (sonst generisches Regelwerk, z. B. wenn Rad nur unterstützende Nebensportart zu einem Laufziel ist) und gibt phasenabhängig vor, welcher Intervalltyp diese Woche angemessen ist (angelehnt an Coggan/Allen-Leistungszonen fürs Rad, Daniels/Pfitzinger-Tempoarbeit fürs Laufen, Seiler-Polarisierungsforschung für den 80/20-Zweig), bewusst ohne harte Prozent-Zonen-Vorgabe pro Woche (ohne Streckenprofil/Wattmessung pro Einheit zu starr) — stattdessen strukturelle Vorgabe analog zum bestehenden Detailgrad der anderen harten Plan-Regeln (z. B. Rad-Grundlage: „überwiegend Z2, keine VO2max/anaerobe Intervalle", Rad-Aufbau: „1-2× Schwellen-/VO2max-Intervalltraining pro Woche", Rad-Wettkampfvorbereitung: „renn-spezifische Intervalle, z. B. Kletterintervalle bei Höhenmeter-Zielen"; analoge Regeln fürs Laufen mit Tempo-/Schwellenläufen statt Sweet-Spot/VO2max-Terminologie). Neue Funktion `determineTrainingPhilosophy(sportType, hoursPerWeek)` (`coachContext.ts`) wählt aus dem neuen, optionalen `SportConfig.hours_per_week`-Feld (`Profile.tsx`-Sportarten-Akkordeon, nur Rad/Lauf) zwischen „Polarisiert (80/20)" und einem effizienzfokussierten Ansatz („Sweet-Spot-Base" beim Rad, „Schwellen-fokussiert" beim Laufen) — Schwellenwert Rad 8h/Woche, Laufen 5h/Woche (niedriger angesetzt wegen höherer Gelenk-/Sehnenbelastung pro Stunde ohne den Impact-Vorteil des Rads), ohne hinterlegte Stundenzahl keine Philosophie-Vorgabe (kein Raten aus fehlender Information). Im selben Zug Taper-Regel in `COACHING-PRINZIPIEN` (`coachPrompt.ts`, Punkt 8) korrigiert: vorher fix „-30% Volumen Woche 13, -50% Woche 14" (unpassend für alles außer ~14-Wochen-Blöcke, z. B. einen 24-26-wöchigen Rad-Vorbereitungsblock) — jetzt relativ zum echten A-Event-Datum formuliert (letzte 2 Wochen vor dem Event, vorletzte ca. -30%, letzte ca. -50%), gilt sportartübergreifend, siehe Kapitel 10 „Plan-Generierung", Kapitel 12 „Coach-Prompts" und Kapitel 5 „athletes"/„season_goals"; davor Feature: Intervallstruktur-Erkennung in der Aktivitäts-Analyse ohne brauchbare Rundendaten (Lauf + Rad) — `isGenericAutoSplit(laps)` (`activityAnalysis.ts`) erkennt geräteseitige Auto-Laps (feste 1km-/Zeit-Splits, keine echten Intervall-Runden: alle Runden bis auf die letzte mit <5% Schwankung in Distanz ODER Dauer) und verwirft sie als für eine Intervallanalyse ungeeignet. Fehlen brauchbare Runden (keine oder nur generische Auto-Splits) UND der Plantag gibt Z4/Z5 vor ODER die Aktivität ist manuell als Intervalltraining markiert (neue Spalte `activities.is_interval`, Toggle „Intervalltraining?" in `ActivityDetail.tsx`, nur Lauf/Rad sichtbar, speichert sofort + stößt automatisch eine Neu-Analyse an), segmentiert `detectEffortSegments(data, sport)` den ohnehin vorhandenen Sekundenverlauf (Tempo/Watt + Puls) deterministisch in Belastungs-/Erholungsblöcke — Schwelle ist die Mitte zwischen 25.- und 75.-Perzentil des Effort-Werts DIESER Aktivität selbst (keine festen Zonen-Grenzen). `formatDetectedIntervals()` gibt erst ab ≥2 erkannten Wiederholungen eine „Intervall N"-Liste aus (sonst `null`, keine geratene Zusatzinfo); ersetzt den ursprünglichen Ansatz, Claude eine rohe Sekundentakt-Liste zum Selbst-Erkennen zu geben (war ungenau: ein echtes 6×1km-Schwellentraining wurde als „5 Blöcke à 7-15 Min" statt 6 Wiederholungen gelesen). Beim Testen zusätzlich aufgefallen und mitbehoben: neuer Prompt-Abschnitt „EMPFEHLUNGEN MÜSSEN ZUM PLAN PASSEN" (`coachPrompt.ts`) verlangt den Abgleich jeder Coach-Empfehlung mit dem `[AKTUELLER WOCHENPLAN]`-Kontext für den betreffenden Tag UND die explizite Wochentag-Nennung bei Empfehlungen für einen anderen Tag als heute (realer Bug: ein für morgen geplanter Lauf wurde als „heute" empfohlen, obwohl heute laut Plan ein Ruhetag war), siehe Kapitel 5 „activities", Kapitel 9 „ActivityDetail.tsx" und Kapitel 12 „Coach-Prompts"; davor Fix: Saisonziel-Woche und Review-Einschränkungen hart in der Wochenplan-Generierung abgesichert — zwei reale Fehlplanungen behoben: (1) `checkGoalPlacement(planJson, monday, goals)` (`WeeklyPlan.tsx`) prüft nach der Generierung, zusätzlich zu `validateConstraints()` und vor dem Speichern, rein anhand von `event_date` (nicht Claudes eigener Datumsangabe), ob ein im Plan erwähntes Saisonziel am richtigen Wochentag steht (Vorfall: ein Wettkampf landete trotz korrekt genanntem Datum eine Woche zu früh, weil Claude Wochentag↔Datum selbst falsch verrechnet hatte); neuer Prompt-Abschnitt `goalWeekSection` berechnet zusätzlich fest (nicht von Claude), ob/an welchem Wochentag ein aktives Saisonziel in die aktuelle Planwoche fällt. (2) Wochenreview (`startReview()`/`saveReviewData()`) liefert jetzt zusätzlich zum Review-Text ein `constraint`-Feld im JSON (aktives körperliches Problem oder `null`); gesetzt landet es als eigener `coach_decisions`-Eintrag (`decision_type: 'review_constraint'`, Fenster über `created_at` statt Aktivitätsdatum) in derselben „AKTUELLE ERHOLUNGS-EINSCHRÄNKUNGEN"-Höchste-Priorität-Regel wie automatisch erkannte `recovery_required`-Signale, statt nur unpriorisiert im `review_notes`-Fließtext unterzugehen; siehe Kapitel 10 „Plan-Generierung" und „Wochenreview"; davor Feature: Grafik zur Entwicklung der geschätzten Zielzeit auf `GoalDetail.tsx` — `calculateFinishTimeProgression()` (neu, `coachContext.ts`) rechnet die bestehende Zielzeit-Schätzung (Effizienz-Methode → Distanz-Anker-Fallback) rückwirkend an 8 Zeitpunkten im 14-Tage-Abstand (letzte 98 Tage) neu, jeweils nur mit den Läufen/Strava-PRs, die zum jeweiligen Auswertungszeitpunkt bereits vorlagen (eigenes 180-Tage-Fenster pro Punkt, PRs nach ihrem `at`-Datum gefiltert), damit kein späterer Formfortschritt fälschlich in frühere Punkte durchsickert; HF-Profil wird als aktuell für alle Zeitpunkte angenommen. `GoalDetail.tsx` zeigt das als Recharts-`LineChart` mit invertierter Y-Achse (oben = schneller), ab ≥2 Punkten sichtbar, siehe Kapitel 9 „GoalDetail.tsx"; davor Bugfix: Wochenplan zeigte eine eingefrorene, teils erfundene Trainingsphase statt der live berechneten — `GoalDetail.tsx` berechnet die Trainingsphase seit dem 15.8. live über `calculateSeasonPhase()`, `WeeklyPlan.tsx` zeigte stattdessen weiterhin Claudes einmalig bei Plan-Generierung geschriebenen Freitext im `plan_json.summary`-Feld, der teils eine erfundene „Phase X Woche Y"-Aussage enthielt (kein Wochenzähler existiert im Code) und mit der Zeit von der live berechneten Phase abweichen konnte, sobald die Wochen-bis-Event-Schwelle in eine neue Phase kippte, ohne dass der Plan neu generiert wurde; `WeeklyPlan.tsx` lädt jetzt selbst das primäre A-Ziel und zeigt die Phase live als eigenes Badge über der Claude-Zusammenfassung (dieselbe `calculateSeasonPhase()`-Funktion, `PhaseResult`-Typ dafür aus `coachContext.ts` exportiert); der `summary`-Prompt (`WeeklyPlan.tsx` **und** `Dashboard.tsx`s Echtzeit-Konflikt-Anpassung, identischer JSON-Schema-Text an beiden Stellen) verbietet Claude jetzt explizit, Phase oder Wochenzahl zu nennen; vor dem Fix generierte Pläne behalten ihren alten Summary-Text bis zur nächsten Neugenerierung (INSERT-only, kein rückwirkendes Update), siehe Kapitel 10 „Plan-Generierung" und Kapitel 9 „GoalDetail.tsx"; davor Feature: Datenbasierte Phasenfortschritts-Beschreibung auf der Ziel-Detailseite statt reiner Kalender-Prozentzahl — `calculatePhaseProgressNarrative()` (neu, `coachContext.ts`) ersetzt die „X% dieser Phase geschafft"-Anzeige, wo möglich, durch einen Satz aus echten Trainingsdaten: **Grundlage** — Ø-Wochenkilometer letzte 4 Wochen vs. die 4 Wochen davor (≥2 Läufe je Fenster nötig); **Wettkampfvorbereitung** — Anteil der Läufe der letzten 4 Wochen mit `avg_hr` ≥ Z3-Untergrenze aus `calculateHRZoneBounds()` (≥3 Läufe mit HF-Daten nötig); **Readaptation** — größte Trainingspause (≥14 Tage) der letzten 180 Tage + Zeitpunkt der Rückkehr, nur relevant wenn ≤30 Tage her; **Taper** liefert bewusst immer `null` (kalendarisch vorgeplante Trainingsreduktion, kein sinnvolles Datensignal). Fehlen die Mindestdatenmengen, fällt `GoalDetail.tsx` auf die alte `progressPct`-Prozentzahl zurück (gleiches Fallback-Prinzip wie bei der Zielzeit-Schätzung), siehe Kapitel 9 „GoalDetail.tsx"; davor Feature: Bereits durchlaufene Trainingsphasen in der Phasen-Balkengrafik grün mit gelbem Stern (`IconStar`) markiert statt wie kommende Phasen grau — macht auf einen Blick sichtbar, wie viele Phasen bereits absolviert wurden, siehe Kapitel 9 „GoalDetail.tsx"; davor Feature: Phasen-Fortschritt in der Balkengrafik der Ziel-Detailseite + anklickbares Erklärungs-Overlay zur Zielzeit-Schätzung — die aktuelle Phase füllt sich anteilig (`progressPct` aus `calculateSeasonPhase()`, berechnet aus der Position innerhalb der Wochen-Grenzen `PHASE_WEEK_BOUNDS` der jeweiligen Phase) statt nur binär aktiv/inaktiv zu sein; Klick auf den `basisText` unter der Zielzeit öffnet ein Overlay mit laienverständlicher Erklärung der jeweils gegriffenen Schätzmethode + 1-2 Quellenangaben (`METHOD_EXPLANATION`, statischer Text, kein Claude-Call), siehe Kapitel 9 „GoalDetail.tsx"; davor Feature: Neue Ziel-Detailseite (`/goals/:id`, verlinkt von einer Ziel-Karte in `Goals.tsx`) mit grafischer Phasen-Anzeige (Readaptation/Grundlage/Wettkampfvorbereitung/Taper aus der bestehenden `calculateSeasonPhase()`-Logik) und, für Laufziele mit hinterlegter Distanz, einer geschätzten Zielzeit inkl. Pace/km — komplett ohne Claude-Call, reine Arithmetik auf echten `activities`/`strava_prs`/HF-Daten. Zwei Schätzmethoden in dieser Reihenfolge: (1) `estimateGoalFinishTimeFromEfficiency()` (primär) — VO2max-Modell nach Daniels & Gilbert (1979) + der ACSM-Beziehung %Herzfrequenzreserve≈%VO2-Reserve, liest die aktuelle aerobe Fitness aus dem Verhältnis von Tempo zu Puls während normaler, durchgehender Läufe (kein Maximaleinsatz nötig — löst damit die Schwäche der reinen Distanz-Hochrechnung in einer Grundlagenphase mit gedrosseltem Pulsbereich, die dort systematisch zu langsame Schätzungen lieferte); (2) `estimateGoalFinishTime()` (Fallback) — Riegel-Hochrechnung vom real gelaufenen Kandidaten (Strava-Bestzeit oder Trainingslauf), dessen Distanz der Zieldistanz am nächsten kommt. Voraussetzung für Methode 1: `saveStravaPrsIfPresent()`/`backfillStravaPrs()` (`strava.ts`) erfassen jetzt alle von Strava erkannten Bestzeiten (5k/10k/15k/Halbmarathon/Marathon/…) statt nur 5k, in `athletes.strava_prs` (JSONB-Map) — generalisiert die vorherige 5k-spezifische `strava_best_5k_seconds/_at`-Speicherung (Spalten bleiben ungenutzt in der DB, nicht gedroppt, um bereits live deployten Code nicht zu brechen), siehe Kapitel 5 „athletes"/„activities" und Kapitel 9 „GoalDetail.tsx"; davor Feature (noch am selben Tag durch die obige Generalisierung abgelöst): Von Strava selbst ermittelte 5k-Bestzeit als zusätzliche Pace-Referenz-Quelle — Strava berechnet `best_efforts` (u.a. „5k") automatisch aus GPS/Zeit-Daten jeder Laufaktivität, `pr_rank=1` markiert die aktuelle athletenweite Bestzeit; Priorität in `buildCoachSystemPrompt()`: manuelle Profil-PB > Strava-PR > Riegel-Schätzung (`estimateBest5kFromActivities()`) als letzter Fallback, `Profile.tsx` zeigte den erkannten Strava-PR als „übernehmen?"-Vorschlag im 5k-Bestzeit-Feld; davor Feature: RPE-Erfassung (Rate of Perceived Exertion) nach Laufeinheiten als Korrektiv zum Stimulus-Check — kurzer 1-10-Zahlen-Picker in `ActivityDetail.tsx` (nur Laufen, Sofort-Speicherung auf Tap, kein Submit-Schritt), fließt in die Claude-Einzelanalyse ein und dient vor allem als Veto für den neuen HF-basierten `stimulus_insufficient`-Check (siehe nächster Punkt): eine hohe RPE bei einer als „zu leicht" geflaggten Einheit widerspricht dem reinen HF-Befund (der durch Hitze, Schlafmangel etc. täuschen kann) und verhindert, dass das Signal als harte Regel in die nächste Plan-Generierung einfließt — `WeeklyPlan.tsx`s Stimulus-Signale-Query filtert serverseitig über `.or('rpe.is.null,rpe.lte.6', {foreignTable: 'activities'})`, siehe Kapitel 5 „activities" und Kapitel 9 „ActivityDetail.tsx"; davor Feature: Erkennung von unzureichendem Trainingsreiz (Ø-HF strukturell unter der geplanten Zielzone) — neuer deterministischer Check `triggerStimulusCheck()` (`src/lib/activityAnalysis.ts`, kein Claude-Call, reiner Ist/Soll-Vergleich): bei einem Lauf ≥15 Min mit geplanter Z2/Z3-Zone, dessen tatsächliche Ø-HF mehr als 5 bpm unter der Zonen-Untergrenze (`calculateHRZoneBounds()`) bleibt, wird ein `coach_decisions`-Eintrag (`decision_type: 'stimulus_insufficient'`) angelegt und als harte Regel in die nächste Wochenplan-Generierung injiziert — spiegelt den bestehenden `recovery_required`-Mechanismus, schließt aber die Lücke, dass bisher nur die Pace-Referenz (`calculateDynamicZ2Pace()`) datengetrieben war, nicht die Reaktion auf zu leichte Einheiten; `activities.stimulus_checked` markiert nach dem ersten Check unabhängig vom Ergebnis (Bugfix-Wiederholungsschutz-Pattern wie `recovery_checked`), siehe Kapitel 5 „activities", Kapitel 10 „Plan-Generierung" und Kapitel 9 „Auto-Analyse"; davor Bugfix: Chat lädt jetzt die neuesten statt der ältesten Nachrichten, mehr Chat-Kontext für den Coach — `Chat.tsx` holte Nachrichten bislang aufsteigend sortiert mit `limit(50)`; bei mehr als 50 Nachrichten in einem Thread wurden dauerhaft die **ältesten** 50 angezeigt, neue Nachrichten schienen nach dem Senden zu verschwinden. Jetzt `order('created_at', {ascending: false}).limit(50)` + `.reverse()` für die chronologische Anzeige. Zusätzlich berücksichtigt `buildCoachContext()` jetzt die letzten 20 statt 10 Chat-Nachrichten (Abschnitt „AKTUELLE CHAT-SESSION") für mehr Gesprächskontinuität, siehe Kapitel 9 „Chat.tsx" und Kapitel 11; davor Feature: Coach-Empfehlung aus dem Chat gezielt in den Wochenplan übernehmen — analog zum bestehenden „Empfehlung übernehmen"-Button in `ActivityDetail.tsx`, jetzt auch im globalen Coach-Chat: Button „In Plan übernehmen" unter der letzten Coach-Nachricht. `extractChatPlanRecommendation()` (neu, `src/lib/planRecommendation.ts`) extrahiert Tag/Dauer/Distanz/Intensität/Beschreibung **und** die Sportart strukturiert per eigenem `/api/analyse`-Call aus der Chat-Nachricht (anders als bei der Aktivitäts-Analyse ist die Sportart hier vorher nicht bekannt); Kraft- oder unklare Nachrichten liefern `sport: null` und werden mit Fehlermeldung abgelehnt, da Kraft-Tage nur feste „Workout I/II/III"-Namen tragen dürfen. `applyPlanRecommendation()` wurde generalisiert (`activityName`/`activityId` → optionales `source`-Freitextlabel wie „Chat-Empfehlung", `activityId` bleibt optional), damit sie von Aktivitäts-Analyse **und** Chat aus nutzbar ist — weiterhin INSERT-only als neue Plan-Version + `coach_decisions`-Audit-Eintrag, siehe Kapitel 9 „Chat.tsx" und „ActivityDetail.tsx — Empfehlung übernehmen"; davor Bugfix: Coach behauptet im Chat nicht mehr fälschlich, den Plan bereits gespeichert zu haben — `Chat.tsx` schreibt technisch nie in `weekly_plans` (nur `chat_messages`), trotzdem bestätigte der Coach auf Bitten wie „übernimm das in den Plan" mit „Plan aktualisiert ✅", ohne dass real etwas gespeichert wurde. Neuer Prompt-Abschnitt „WOCHENPLAN-ÄNDERUNGEN IM CHAT" in `coachPrompt.ts` (`buildCoachSystemPrompt()`, wirkt global für alle Claude-Calls inkl. Chat) verpflichtet den Coach stattdessen, die Empfehlung konkret zu formulieren und auf den „In Plan übernehmen"-Button zu verweisen, siehe Kapitel 12 und Kapitel 18.3; davor Bugfix: Chat-Historie fließt jetzt in Wochenplan-Generierung und -Review ein — `buildCoachContext()` wurde in `generatePlan()`/`startReview()` (`WeeklyPlan.tsx`) bisher **ohne** `threadId` aufgerufen, wodurch die Chat-Session-Sektion des Kontexts (Abschnitt „AKTUELLE CHAT-SESSION") immer leer blieb — Wünsche/Absprachen aus dem Coach-Chat hatten dadurch keinen Einfluss auf den generierten Plan. Beide Aufrufe übergeben jetzt `threadId = athlete.id` mit; kein hartes Erzwingen, Claude gewichtet den Chat-Kontext nur als ein Signal von mehreren, siehe Kapitel 10 „Plan-Generierung" und Kapitel 11; davor Bugfix: Push-Abmeldung überlebt jetzt Logout und iOS-Icon-Reinstall — das Opt-out-Flag lag bisher nur in `localStorage`, das sowohl bei Logout als auch beim iOS-Trick „Icon entfernen + neu hinzufügen" geleert wird; da `Notification.permission` dabei `granted` bleibt (kann JS nicht zurücksetzen), machte `syncPushSubscription()` einen bewussten Abmelde-Klick beim nächsten App-Start klaglos rückgängig. Neue Spalte `athletes.push_opted_out` (serverseitig) ersetzt den lokalen `OPT_OUT_KEY`; `syncPushSubscription(athleteId, pushOptedOut)` bekommt das Flag jetzt als Parameter aus der beim App-Start ohnehin geladenen Athleten-Row, siehe Kapitel 16 „Push Notifications"; davor Feature: Coach-Empfehlung aus einer Aktivitäts-Analyse gezielt in den Wochenplan übernehmen — neuer Button „Empfehlung übernehmen" in `ActivityDetail.tsx` (nur Lauf/Rad, Kraft-Tage tragen einen festen „Workout I/II/III"-Namen); `extractPlanRecommendation()` (`src/lib/planRecommendation.ts`) extrahiert Tag/Dauer/Distanz/Intensität/Beschreibung strukturiert per eigenem `/api/analyse`-Call aus `claude_analysis`, Zielwoche wird deterministisch aus dem genannten Wochentag berechnet statt von Claude geraten; Bestätigungsdialog mit editierbaren Feldern, Tag-Dropdown nur mit zur Sportart passenden Tagen (`dayMatchesSport()`); `applyPlanRecommendation()` aktualisiert gezielt nur den betroffenen Tag (nie die Trainingstag-Struktur) über neues `insertPlanVersion()` (`src/lib/weeklyPlan.ts`, geteilte INSERT-only-Speicherlogik, aus `WeeklyPlan.tsx`s `saveManualPlanChange()` extrahiert), plus `coach_decisions`-Audit-Eintrag; verifiziert per Wegwerf-Testathlet via Playwright, siehe Kapitel 9 „ActivityDetail.tsx — Empfehlung übernehmen"; davor Bugfix: ICS-Kalenderabo listete alle `weekly_plans`-Wochen inklusive Vergangenheit statt nur ab der aktuellen Woche — `buildIcsFeed()` (`api/_lib/ics.ts`) filtert Pläne jetzt über neue Hilfsfunktion `currentWeekStartVienna()` (Montag der laufenden Woche in Europe/Vienna, analog `viennaTodayInfo()` in `api/send-daily-reminder.ts`) heraus, bevor Events gebaut werden; betrifft sowohl die Vercel-Function als auch die lokale Vite-Dev-Middleware, da beide dieselbe Funktion aufrufen, siehe Kapitel 7 „GET /api/calendar/[athleteId]" und Kapitel 16 „Kalender-Export"; davor Bugfix: Kilometer-Splits im Claude-Analyse-Prompt nutzten `elapsed_time` statt `moving_time` — bei einer Pause innerhalb eines Splits (z. B. GPS-Lock am Start) nannte der Analysetext eine andere Kilometerzeit/-pace als die Kilometer-Splits-Tabelle in `ActivityDetail.tsx`, obwohl beide auf demselben `splits_metric_json` basieren; behoben durch Angleichung an die UI (`moving_time`), siehe Kapitel 9 „Auto-Analyse"; davor Bugfix: Aktivitätsdauer ab 60 Minuten wurde in Prompt-Texten (Wochenreview, Aktivitäts-Analyse, Echtzeit-Alert-Konfliktcheck) und zwei UI-Anzeigen in `WeeklyPlan.tsx` als rohe Minutenzahl geschrieben (z. B. „386 Minuten" statt „6 Std 26 Min"); neuer gemeinsamer Helper `formatDurationHuman(seconds)` (`src/lib/dateUtils.ts`, `< 60 min` → `"X min"`, sonst `"H Std M Min"`) ersetzt die verstreuten `Math.round(duration_s / 60)`-Stellen, siehe Kapitel 9 „Auto-Analyse"; davor Feature: Swipe-Navigation zwischen den BottomNav-Tabs — horizontales Wischen (links → nächster, rechts → vorheriger Tab, kein Wrap-Around an den Rändern) auf den 5 Haupt-Tabs; Tab-Reihenfolge/-Sichtbarkeit war bisher inline in `BottomNav.tsx` berechnet, jetzt in neuen Hook `useVisibleTabs()` (`src/lib/`) extrahiert als gemeinsame Quelle der Wahrheit, genutzt von `BottomNav.tsx` (Rendering) und dem neuen `useTabSwipeNavigation()` (native `touchstart`/`touchend`, keine neue Abhängigkeit — Schwelle horizontale Distanz >60px UND |Δx|>2×|Δy|); registriert im Layout-Wrapper (`App.tsx`), aktiv aber nur, wenn der aktuelle Pfad tatsächlich einem der 5 Haupt-Tabs entspricht — auf `/activity/:id`, `/onboarding`, `/auth/callback` und `/` dadurch automatisch inert, ohne dass `App.tsx` das explizit steuern muss; `DayCard` in `WeeklyPlan.tsx` trägt neu `data-swipe-ignore`, damit Gesten, die auf einer Karte oder deren Drag-Griff beginnen, die Swipe-Erkennung komplett überspringen (Kollisionsvermeidung mit dnd-kit-Drag und Long-Press-Kontextmenü), siehe Kapitel 3 „lib/"; davor Bugfix: „Zurück"-Button in `ActivityDetail.tsx` führte immer zum Dashboard statt zur tatsächlich vorherigen Seite — war hart als `<Link to="/dashboard">` verdrahtet statt echtes History-Back zu nutzen; behoben durch `navigate(-1)` (react-router), abgesichert durch einen Guard (`window.history.state?.idx > 0`), der bei Deep-Links ohne SPA-eigene History (direkter URL-Aufruf, Push-Benachrichtigung) stattdessen auf `navigate('/dashboard')` zurückfällt, damit `navigate(-1)` nicht aus der App heraus navigiert; dabei zusätzlich aufgefallen und mitbehoben: der Dashboard-Sportart-Filter (`Dashboard.tsx`, `filter`-State) war reiner `useState` und wurde beim Unmount/Remount der Route (jeder Wechsel zu `/activity/:id` unmountet `Dashboard` komplett) ohnehin auf `null` zurückgesetzt — unabhängig von der Navigationsmethode; jetzt zusätzlich in `sessionStorage` (`dashboard_filter`) gespiegelt, damit „Dashboard mit Filter → Aktivität öffnen → Zurück" den Filter tatsächlich erhält, siehe Kapitel 9 „ActivityDetail.tsx — Zurück-Button" und „Dashboard.tsx — Filter-Persistenz"; davor Feature: Self-Service Konto-Löschung — neuer rot abgesetzter Button „Konto löschen" ganz unten in `Profile.tsx` mit Bottom-Sheet-Bestätigungsmodal (Zwei-Stufen-Bestätigung); Ablauf: `deauthorizeStrava()` (neu, `src/lib/strava.ts`, ruft `/api/strava-token` mit neuem `grant_type: 'deauthorize'` auf, proxied zu `strava.com/oauth/deauthorize`, fehlertolerant) trennt zuerst die Strava-Verbindung, danach löscht die neue Postgres-Funktion `delete_athlete_account(p_athlete_id UUID)` (SECURITY DEFINER, siehe Kapitel 18) in einer Transaktion `chat_messages`→`coach_decisions`→`weekly_plans`→`activities`→`season_goals`→`athletes`; `p_athlete_id` kommt ausschließlich aus dem geladenen Session-Athleten; schlägt die DB-Löschung fehl, automatischer Rollback + Fehleranzeige im Modal ohne Redirect, schlägt nur die Strava-Trennung fehl, Hinweis auf manuelles Trennen unter strava.com/settings/apps vor dem finalen Logout-Redirect; Anlass: zweiter echter Athlet mit echten personenbezogenen Daten in der App, bisher liefen Löschungen nur manuell direkt in Supabase; verifiziert per Test-Athlet mit Daten in allen 6 Tabellen — SQL bestätigt vollständige Löschung ohne Kollateralschaden an den beiden echten Athleten, siehe Kapitel 9 „Profile.tsx — Konto löschen" und Kapitel 18; davor Bugfix: Echtzeit-Alert-Banner erschien nach echtem App-Neustart erneut, obwohl bereits per "Verwerfen" weggeklickt — das bisherige `sessionStorage`-Gate (`peakform_alert_{weekStart}`) überlebte keinen Prozess-Neustart (v.a. iOS PWA), "Verwerfen" persistierte nichts Dauerhaftes; behoben durch neuen `coach_decisions`-Eintrag `decision_type='realtime_alert_dismissed'` mit `related_plan_id` = aktuelle `weekly_plans.id` beim Klick auf "Verwerfen", Konflikt-Check in `Dashboard.tsx` überspringt Claude-Call komplett, sobald für die geladene Plan-Version bereits ein solcher Eintrag existiert; `sessionStorage`-Gate vollständig entfernt; da der Dismiss an die konkrete Plan-`id` statt an `week_start` gebunden ist, macht jede neue Plan-Version (Plan anpassen/Review/manuelle Änderung) den alten Dismiss automatisch obsolet; verifiziert per SQL-Transaktion (`BEGIN…ROLLBACK`, kein Testdaten-Rückstand) gegen die Produktions-DB, siehe Kapitel 18.3 „Bugfix 12. Juli 2026"; davor Bugfix: doppelte automatische Aktivitäts-Analyse bei gleichzeitigen Syncs — per Live-Test (Dummy-Aktivität mit `claude_analysis = NULL`, zwei parallele Seitenaufrufe) verifiziert, dass React StrictMode's Dev-Doppel-Mount des Dashboard-`useEffect` sowie ein zeitgleicher Sync aus `WeeklyPlan.tsx` dieselbe unanalysierte Aktivität parallel aufgreifen und zwei echte Claude-Calls dafür auslösen konnten; behoben durch neue Spalte `activities.analysis_claimed_at` + `claimActivityForAnalysis(activityId)` (neu, `src/lib/activityAnalysis.ts`) — atomares conditional `UPDATE ... WHERE claude_analysis IS NULL AND (analysis_claimed_at IS NULL OR abgelaufen nach 2 Min)`, das sowohl der Sync-Hintergrundjob in `syncActivitiesToSupabase()` als auch der Fallback `closeOutstandingAnalyses()` vor jedem `analyzeActivity()`-Aufruf durchlaufen; nur ein konkurrierender Aufruf gewinnt den Claim, der Verlierer überspringt die Aktivität; Claim wird nach Erfolg/Fehlschlag zurückgesetzt, ein verwaister Claim (z. B. Tab während der Analyse geschlossen) heilt nach 2 Minuten selbstständig; der manuelle „Neu analysieren"-Button umgeht den Claim bewusst, siehe Kapitel 5 „activities", Kapitel 9 „Auto-Analyse — Race-Fix" und Kapitel 10 „Fallback: `closeOutstandingAnalyses()`"; davor Feature: „Plan generieren"-Button für bereits abgelaufene Wochen entfernt — neue Variable `isPastWeek = monday < getISOMonday(new Date())` in `WeeklyPlan.tsx` (gleiches Vergleichsmuster wie die bestehende Wochenreview-Sichtbarkeit `monday <= getISOMonday(new Date())`, dort unverändert); bei `isPastWeek` entfällt der Button ersatzlos — ohne Plan erscheint stattdessen der neutrale Hinweistext „Für diese Woche wurde kein Plan erstellt", mit Plan gar nichts (der Plan wird oberhalb ja bereits normal angezeigt); aktuelle und zukünftige Wochen zeigen den Button unverändert mit bisheriger Label-Logik („Plan für diese Woche generieren" ohne Plan, „Plan neu generieren" mit Plan); verhindert ausschließlich das rückwirkende Erzeugen NEUER Pläne für vergangene Wochen, siehe Kapitel 10 „Button-Sichtbarkeit — Plan generieren"; davor Refactor: Wochenreview und Plan-Generierung vollständig entkoppelt — `startReview()` erzeugt ab jetzt ausschließlich eine Bewertung der abgelaufenen Woche (`{ review: string }`, `max_tokens` 3000→600), keinen `next_week_plan` mehr; alle Sportarten-/Trainingstage-Constraints und das SELF-CHECK für den Folgeplan wurden aus dem Review-Prompt entfernt — das bleibt exklusiv Sache von `generatePlan()`; `saveReviewData(reviewText)` legt die neue Version jetzt auf `week_start = weekStr` (die **bewertete** Woche W selbst) statt auf der Folgewoche an, `plan_json` wird dabei unverändert vom Vorgänger übernommen (Spalte ist `NOT NULL` — Guard verhindert Review-Speicherung, falls für die Woche noch gar kein Plan existiert); `hasViolation`-Parameter + Amber-Violation-Banner im Review-Bereich entfernt, da ohne Plan-Generierung keine Constraint-Verletzung mehr möglich ist; `WeeklyReviewCard`-Anzeige auf einen einzigen Check (`plan?.review_notes`) vereinfacht — der separate `nextWeekPlan`-Query (`week_start = W+1`) und die bisherige Fall-A/Fall-B-Unterscheidung sind komplett entfallen; Effekt: „Plan generieren" wird nie mehr implizit durch ein Review ausgelöst, nach einem Review zeigt die Folgewoche „Noch kein Plan für diese Woche", bis der Athlet ihn explizit anstößt; Reviews vor diesem Fix bleiben als bekannte, harmlose Alt-Daten-Inkonsistenz auf `week_start = W+1` liegen (kein Migrations-Fallback), siehe Kapitel 10 „Wochenreview" und „Wochenreview-Ergebnis-Karte"; davor Feature: Wochenreview-Ergebnis-Karte + persistenter User-Freitext — neue Spalte `weekly_plans.review_user_input` speichert ab jetzt den rohen, unveränderten Freitext des Athleten aus dem Review-Formular (bisher nur ephemer im React-State, nie persistiert); `saveReviewData()` (`WeeklyPlan.tsx`) schreibt sie zusätzlich zu `review_notes` im selben INSERT; neue aufklappbare Komponente `WeeklyReviewCard` (Titel „Wochenreview" + Chevron, standardmäßig ausgeklappt, lokaler Expand-State ohne Persistenz) zeigt „Deine Notizen:" (falls vorhanden) + „Coach-Bewertung:" in zwei datenbankgetriebenen Fällen: Fall A — der geladene `plan` der angezeigten Woche trägt bereits `review_notes` (Plan entstand aus dem Review der Vorwoche) → Karte erscheint oberhalb des Plan-Inhalts, ergänzt ihn statt ihn zu ersetzen; Fall B — ein zusätzlich geladener `nextWeekPlan` (Query auf `week_start = W+1`, gleiche UTC-Fallback-Logik wie der Haupt-Plan-Query) trägt `review_notes` (Woche W wurde bereits reviewt) → Karte ersetzt in der Wochenreview-Sektion vollständig das bisherige Eingabe-Formular (Aktivitätsliste + Textarea + Button); `saveReviewData()` setzt `nextWeekPlan` direkt nach dem Insert, wodurch Fall B ohne Reload greift; Legacy-Reviews vor diesem Fix (`review_user_input IS NULL`) zeigen nur die Coach-Bewertung, „Deine Notizen"-Bereich bleibt ausgeblendet; ersetzt die bisherige rein ephemere `reviewResult`-Anzeige, die beim Wochenwechsel/Reload verlorenging, siehe Kapitel 5 „weekly_plans" und Kapitel 10 „Wochenreview-Ergebnis-Karte"; davor Feature: Roast-Me-Freischalt-Logik — neue Athleten müssen seit dem Onboarding (`athletes.created_at`) mindestens 3 eigene Aktivitäten synchronisiert haben, bevor „Roast Me" nutzbar wird; historische, beim allerersten Strava-Sync automatisch mitimportierte alte Aktivitäten zählen nicht mit. `checkRoastUnlock(athleteId, createdAt)` (lokal in `ActivityDetail.tsx`) zählt `activities` mit `date >= createdAt`, Ergebnis `{unlocked, remaining}` im neuen State `roastUnlock`, einmalig beim Laden aufgerufen; Button bleibt bei `unlocked: false` sichtbar und klickbar, aber grau (`bg-slate-600 opacity-40 cursor-not-allowed`), Klick zeigt statt Claude-Call einen 2,5s-Toast mit exakter Restanzahl; Bestandsuser mit längst erreichter Schwelle sehen keinerlei Unterschied, siehe Kapitel 9 „ActivityDetail.tsx — Roast Me"; davor Feature: Dynamische Z2-Pace-Kalibrierung aus echten Läufen — `calculateDynamicZ2Pace(runningActivities, hrZoneMin, hrZoneMax)` (neu, `coachContext.ts`) filtert die letzten Laufaktivitäten auf solche mit `avg_hr` innerhalb der Z2-HF-Range (+3/−5 bpm Toleranz) und vorhandenen `distance_m`/`duration_s`, nimmt davon die letzten 8 und berechnet einen distanzgewichteten Pace-Durchschnitt (`totalDurationSec / totalDistanceKm`, nicht den einfachen Mittelwert der Einzel-Paces, damit längere Läufe stärker einfließen); ab mindestens 3 qualifizierenden Läufen liefert sie `{paceSecPerKm, basedOnRuns}`, sonst `null` (Fallback); `calculateHRZones()` wurde dafür refaktoriert — die Z2-Grenzen werden jetzt über die neue Hilfsfunktion `calculateZ2HRRange(maxHR, restingHR?)` (Karvonen-Methode wenn `restingHR` vorhanden, sonst %-Methode) berechnet und sowohl für den Zonen-Text als auch numerisch weiterverwendet, keine doppelte Formel mehr; `calculatePaceReference()` bekommt einen neuen optionalen 3. Parameter `dynamicZ2` — ist er gesetzt, wird die „Z2 Trainingspace"-Zeile aus `dynamicZ2.paceSecPerKm ± 15s` gebaut und mit „(aus deinen letzten N Läufen berechnet, nicht aus 5k-PB geschätzt)" gekennzeichnet, sonst bleibt die bisherige Formel (`best5kSeconds/5 × 1.15–1.30`) als Fallback aktiv; Zielpace und Schwellenpace bleiben in jedem Fall unverändert aus der 5k-Bestzeit berechnet, da sie Zielwerte für ein zukünftiges Event sind, keine Ist-Werte der aktuellen Form; `buildCoachSystemPrompt()` (`coachPrompt.ts`) lädt dafür zusätzlich die letzten 30 Läufe (`Run`/`VirtualRun`/`TrailRun`, neueste zuerst) parallel zu Athlet/A-Event und übergibt das Ergebnis von `calculateDynamicZ2Pace()` an `calculatePaceReference()` — behebt das Problem, dass die rein formelbasierte Z2-Pace die aktuelle Form regelmäßig überschätzte, siehe Kapitel 12 und Kapitel 18 „Coach-System"; davor Feature: Vorgezogene Trainingstage erkennen und verknüpfen — `matchActivityToDay()` erkennt jetzt an **jedem** Tag (nicht mehr nur an Ruhetagen) eine `extraActivity`, deren Sportart nicht zum geplanten `dayPlan.type` passt, unabhängig vom regulären Status (`completed`/`missed`/`pending`); `DayCard` zeigt dafür ein „+1"-Badge + Zeile „Außerdem: {name}"; ein zentrales `dayMatches`-Memo ersetzt die bisher dreifache Matching-Berechnung (Wochen-Kennzahlen, DayCard-Rendering, neue Erkennung); darauf aufbauend erkennt `pickupSuggestion`, ob eine `extraActivity` an Tag A zur noch unerledigten (`pending`) Sportart eines anderen Tages B passt, und zeigt einen dismissable, neutral-blauen Banner „Du hast dein {Sportart} für {Tag} bereits heute/am {Tag} gemacht — als erfüllt markieren?" mit „Verknüpfen"/„Nein danke" (Ablehnung wird pro `stravaId`+Zieltag in `dismissedPickupKey` gemerkt, Reset bei Wochenwechsel); Bestätigung setzt `DayPlan._fulfilledBy = {date, stravaId}` auf Tag B — der Tag-Inhalt selbst bleibt unverändert — und persistiert über denselben `applyManualEdit()`/`saveManualPlanChange()`-Weg wie Swap/Ruhetag (INSERT-only, `change_reason` z. B. „Krafttraining von So auf Sa vorgezogen", `coach_decisions`-Eintrag `decision_type='manual_plan_edit'` automatisch); `matchActivityToDay()` behandelt einen Tag mit gesetztem `_fulfilledBy` als `completed` und löst `activity` über die verknüpfte `stravaId` auf statt über den Kalendertag — `DayCard` zeigt zusätzlich „Vorgezogen am {Wochentag, Datum}", Tap navigiert zur echten Aktivität; Long-Press-Kontextmenü bekommt bei gesetztem `_fulfilledBy` den Eintrag „Verknüpfung aufheben" (entfernt das Feld wieder); Wochen-Kennzahlen und Wochenreview zählen den Tag automatisch korrekt als absolviert, da beide auf demselben `dayMatches`-Status bzw. der rohen Aktivitätsliste basieren, keine separate kalenderbasierte Zählung — siehe Kapitel 10 „Manuelles Vorziehen erkannter Aktivitäten"; davor Drei Korrekturen am manuellen Verschieben von Trainingstagen in `WeeklyPlan.tsx`: (1) Bugfix Persistenz — `commitManualChange()` las den zu speichernden Plan bisher aus dem `manualPlanJson`-State statt aus einem Parameter; da der No-Konflikt-Pfad `commitManualChange()` synchron im selben Tick wie `setManualPlanJson()` aufrief, griff dort eine Stale Closure — die allererste manuelle Änderung nach Seitenaufruf wurde dadurch gar nicht gespeichert (stiller Totalverlust), jede weitere speicherte die vorherige statt der aktuellen Änderung; behoben durch einheitliche Signatur `commitManualChange(updatedPlan, changeReason, hasViolation)`, die den Plan in jedem Aufrufpfad explizit als Parameter bekommt; (2) der „•••"-Button wurde wieder entfernt, Kontextmenü öffnet wieder per 500ms-Long-Press auf die Karte (funktioniert jetzt konfliktfrei, da nur der dedizierte Drag-Griff auf Pointer-Events für den Sortier-Drag reagiert), Menü auf zwei Einträge reduziert (Ruhetag-Eintrag + „Verschieben nach...", „Details anzeigen" entfernt); (3) „Als Ruhetag markieren" verwirft den ursprünglichen Taginhalt nicht mehr, sondern speichert ihn eingebettet in `_restoreFrom` — das Menü zeigt bei manuell erzeugten Ruhetagen stattdessen „Aktivität wiederherstellen", bei echten (vom Coach geplanten) Ruhetagen weiterhin nichts; Details siehe Kapitel 10 „Manuelles Verschieben von Trainingstagen"; davor Bugfix: PointerSensor-`distance`-Schwelle (8px) beim Drag-and-Drop führte auf Mobile dazu, dass normales vertikales Scrollen fälschlich als Drag-Start erkannt wurde — betraf sowohl die Wochenplan-Tage (`WeeklyPlan.tsx`) als auch das Muskelgruppen-Ranking (`Profile.tsx`); behoben durch zwei Maßnahmen: (1) `activationConstraint` von `{distance: 8}` auf `{delay: 200, tolerance: 8}` umgestellt (200ms bewusstes Halten statt sofortiger 8px-Bewegung startet den Drag) und (2) expliziter Drag-Griff (`IconGrip`) eingeführt — `attributes`/`listeners` aus `useSortable()` hängen jetzt nur noch am Griff-Button, nicht mehr an der ganzen Karte/Zeile, wodurch der Rest der Fläche uneingeschränkt scrollbar bleibt (kein `touch-none` außerhalb des Griffs); zusätzlich wurde das 500ms-Long-Press-Kontextmenü in `WeeklyPlan.tsx` durch einen expliziten „•••"-Button (`IconMore`, neuer Export in `icons.ts`) ersetzt, der das bestehende Bottom-Sheet per Tap statt Halten öffnet — vermeidet Kollision mit der neuen delay-Aktivierung, siehe Kapitel 10 „Manuelles Verschieben von Trainingstagen" und Kapitel 9 „Profile.tsx" (Teil B — Körperziele); davor Feature: Manuelles Verschieben von Trainingstagen im laufenden Wochenplan — Drag-and-Drop-Tausch zweier Tage via `@dnd-kit` (`swapDays()`, `SortableDayCard` als Wrapper um `DayCard`, gleiches Sensor-/Drag-Optik-Setup wie beim Ästhetik-Ranking in Profile.tsx), Long-Press-Kontextmenü (500ms, 8px Bewegungstoleranz — eigene Pointer-Handler koexistieren mit dem dnd-kit-Sensor) mit den drei Optionen „Als Ruhetag markieren"/„Verschieben nach..."/„Details anzeigen" als Bottom-Sheet (gleicher Stil wie das Mid-Week-Feedback-Modal), client-seitige Konflikt-Prüfung `checkPlanConflicts()` (reine Funktion, kein Claude-Call — nutzt dieselbe „intensiv"-Definition wie der `generatePlan()`-Prompt: Z3+-Ausdauer und Krafttraining zählen beide als intensiv) mit nicht-blockierendem Amber-Banner („Abbrechen"/„Trotzdem speichern") bei Konflikt bzw. direktem Speichern + Toast bei keinem Konflikt, Persistierung über `saveManualPlanChange()` mit derselben INSERT-only-Versionierung wie `savePlanJson()`/`saveReviewData()` inkl. neuem `coach_decisions`-Eintrag `decision_type='manual_plan_edit'` — siehe Kapitel 10 „Manuelles Verschieben von Trainingstagen"; davor Bugfix: iOS-Safari zoomte beim Fokussieren von Eingabefeldern automatisch in die Seite hinein, weil Safari das bei jeder Schriftgröße unter 16px auslöst — betraf **alle** `<input>`/`<textarea>`/`<select>`-Elemente der App, da sie durchgängig `text-sm` (14px) statt `text-base` (16px) nutzten; behoben in `ActivityDetail.tsx` (Mid-Week-Feedback-Textarea, siehe Kapitel 9 „ActivityDetail.tsx — Mid-Week-Feedback"), `Profile.tsx`, `Goals.tsx`, `Onboarding.tsx`, `Chat.tsx` und `WeeklyPlan.tsx` (Wochenreview-Feedback) — überall `text-sm` durch `text-base` ersetzt, Layout/Padding/Design sonst unverändert; davor Feature: Pagination „Mehr laden" für die Dashboard-Aktivitätsliste — `fetchRecentActivities()` (`src/lib/strava.ts`) um `page`-Parameter erweitert, paginiert gegen die Strava API statt gegen Supabase (Supabase bleibt reiner Write-Cache), siehe Kapitel 9 „Dashboard.tsx — Pagination"; davor Bugfix: Echtzeit-Alert im Dashboard nannte Aktivitätsdatum mit falschem/widersprüchlichem Wochentag — Konflikt-Check-Prompt nutzte rohes `plan_json` (nur Mo-So-Kürzel) statt über `planJsonWithDates()` (jetzt aus `coachContext.ts` exportiert) mit Kalenderdatum angereichert; siehe Kapitel 18.3 „Bugfix 4. Juli 2026"; davor Feature: Mid-Week-Feedback von WeeklyPlan.tsx nach ActivityDetail.tsx verschoben, dort nebeneinander mit „Roast Me" (`grid grid-cols-2 gap-3`, je 50% Breite) — Ladelogik via `.maybeSingle()` auf `coach_decisions` statt Map, siehe Kapitel 9 „ActivityDetail.tsx — Mid-Week-Feedback" und Kapitel 10 „Mid-Week-Feedback — verschoben nach ActivityDetail.tsx"; Dashboard-Aktivitätskarten zeigen dezenten `IconCommentFilled`-Indikator via Batch-Query (Join über `activities.strava_id`, da `coach_decisions.related_activity_id` auf die Supabase-UUID zeigt), siehe Kapitel 9 „Dashboard.tsx — Feedback-Indikator auf Aktivitätskarten"; `buildRoastPrompt()` (`src/lib/funModePrompts.ts`) um optionalen `userFeedback`-Parameter erweitert — Roast Me nutzt vorhandenes Mid-Week-Feedback schamlos als zusätzliches Roast-Material, bleibt aber vollständig ephemer (kein Zurückschreiben nach `coach_decisions`), siehe Kapitel 9 „ActivityDetail.tsx — Roast Me"; davor Feature: Datumszeile in ActivityDetail.tsx um lokale Startuhrzeit ergänzt — `toLocaleTimeString('de-DE', {hour:'2-digit', minute:'2-digit'})` + " Uhr" hinter dem Datum, z. B. „Donnerstag, 2. Juli 2026 · 06:05 Uhr", siehe Kapitel 9 „ActivityDetail.tsx — Datumszeile"; davor Bugfix: Roast-Me-Ergebnis — `max_tokens` von 300 auf 500 erhöht (Text endete teils mitten im Wort), Auto-Scroll zur Ergebnis-Card via `roastResultRef` + `scrollIntoView({behavior:'smooth', block:'start'})` nach erfolgreichem Claude-Call, siehe Kapitel 9 „ActivityDetail.tsx — Roast Me"; davor Vereinfachung: „Spaß-Analyse" (3 Modi) zu „Roast Me" (1 Modus) reduziert — nur noch `buildRoastPrompt({name, sport})` statt `buildFunModePrompt(mode, ...)`, ein einzelner Button „🔥 Roast Me 🔥" (Flammen-Icons, orange→rot-Gradient) statt 3-Button-Reihe, Ergebnis-Card im Flammen-Look statt fuchsia, Ton bitterböser/South-Park-artig statt nur frech, `IconSarcastic`/`IconSexy` entfernt, siehe Kapitel 9 „ActivityDetail.tsx — Roast Me"; davor Verbesserung: Sexy-Modus in der Spaß-Analyse (`src/lib/funModePrompts.ts`) 1-2 Stufen frecher — forschere, unverblümtere Anspielungen „unter der Gürtellinie" mit direkteren Formulierungen ("tief reingehen", "aufs Tempo drücken", "es noch mal steigern"), Ton bewusst selbstsicher/schlagfertig statt zurückhaltend; Grenze bleibt unverändert hart: reines Wortspiel über Trainingsdaten, keine explizite Beschreibung sexueller Handlungen, kein Kommentar zu Körper/Aussehen; davor Verbesserung: Spaß-Analyse sportart-spezifisch geschärft — alle 3 Modi erzwingen konkrete Zahlen (Pace/Tempo/HF beim Laufen, Watt/Trittfrequenz/HF beim Rad, Gewicht/Wiederholungen/Muskelgruppen bei Kraft) über `sportVocabHint()`, `ActivityDetail.tsx` leitet `sport` aus `activity.type` ab und füttert `buildFunStatsText()` mit sportart-spezifischen Rohdaten statt generischem Block; davor Feature: Spaß-Analyse in ActivityDetail.tsx — optionale, komplett vom Coach-Gedächtnis isolierte KI-Kommentare in 3 Modi (Sarkastisch/Roast/Sexy), erscheint als Button-Reihe unterhalb der ernsten KI-Analyse, rein ephemer im React-State, siehe Kapitel 9 „ActivityDetail.tsx — Spaß-Analyse"; davor UI-Anpassung: Wochen-Kennzahlen-Leiste im Wochenplan — „X / Y Einheiten"-Zeile entfernt, nur noch eine Zeile mit Lauf-/Rad-km + Kraft-Gesamtgewicht, Icons/Text vergrößert (`size={20}`, `text-base`), siehe Kapitel 10 „Wochen-Kennzahlen-Leiste"; davor Feature: Wochen-Kennzahlen-Leiste im Wochenplan eingeführt — Lauf-/Rad-km + Kraft-Gesamtgewicht zwischen Phasen-Banner und DayCards, ausgeblendet bei Wochen ohne jegliche Aktivität; davor Bugfix: Sportart-Icon in der Dashboard-Aktivitätsliste wurde bei langen Aktivitätsnamen durch fehlendes `flex-shrink-0` vom Flexbox-Layout mitgeschrumpft — siehe Kapitel 9 „Dashboard.tsx"; davor Feature: Automatische Aktivitäts-Analyse nach Strava-Sync — kein manueller Klick auf „Analysieren" mehr nötig, Button heißt jetzt „Neu analysieren", Analyse-Logik in `src/lib/activityAnalysis.ts` extrahiert, siehe Kapitel 9 „Auto-Analyse" und Kapitel 10 „Fallback: `closeOutstandingAnalyses()`"; davor Bugfix: Datumsfehler in Coach-Analysen — falsches Aktivitätsdatum bei Mid-Week-Feedback, fehlende Kalenderdaten im Wochenplan-Kontext, UTC-Slice statt Lokalzeit-Formatierung, siehe Kapitel 11 „Bugfix 2. Juli 2026")
...
│   │                         Tab-Liste aus useVisibleTabs() (lib/); horizontales Wischen zwischen Tabs
│   │                         via useTabSwipeNavigation() im Layout-Wrapper (App.tsx), siehe Kapitel 3 lib/
│   ├── pages/
...
│       │                        (Route, Icon, Label, Feature-Gate); einzige Quelle der Wahrheit, genutzt
│       │                        von BottomNav.tsx (Rendering) und useTabSwipeNavigation.ts (Swipe-Ziel)
│       └── useTabSwipeNavigation.ts # useTabSwipeNavigation(): native touchstart/touchend Swipe-Erkennung
│                                zwischen den Tabs aus useVisibleTabs(); Schwelle >60px + |Δx|>2×|Δy|,
│                                links→nächster/rechts→vorheriger Tab, kein Wrap-Around; aktiv nur wenn
│                                der aktuelle Pfad einem der 5 Haupt-Tabs entspricht (sonst inert);
│                                Elemente mit [data-swipe-ignore] (z. B. DayCard in WeeklyPlan.tsx) brechen
│                                die Geste ab, damit dnd-kit-Drag/Long-Press nicht kollidieren
...
sport_types           JSONB                -- Format: [{type, days, hours_per_week?}]
...
best_5k_seconds       INTEGER DEFAULT NULL -- 5k-Bestzeit in Sekunden (manuell im Profil); Basis für Pace-Berechnung
...
strava_prs            JSONB DEFAULT '{}'    -- von Strava selbst erkannte Bestzeiten je Standarddistanz
                                             -- (best_efforts/pr_rank aus der Activity-Detail-Response); Keys
                                             -- sind Stravas Distanz-Namen lowercased ("5k","10k","half-marathon",
                                             -- siehe STANDARD_DISTANCES_KM in coachContext.ts), Werte
                                             -- {seconds, at}. saveStravaPrsIfPresent() (strava.ts) aktualisiert
                                             -- bei jedem erstmaligen Detail-Fetch einer Laufaktivität. Priorität
                                             -- für die 5k-Pace-Referenz in buildCoachSystemPrompt():
                                             -- best_5k_seconds (manuell) > strava_prs['5k'] > Riegel-Schätzung
                                             -- (estimateBest5kFromActivities). Löst die frühere 5k-spezifische
                                             -- strava_best_5k_seconds/_at-Speicherung ab (Spalten bleiben
                                             -- ungenutzt in der DB, nicht gedroppt)
strava_prs_backfill_done BOOLEAN DEFAULT false -- true nach dem einmaligen backfillStravaPrs()-Durchlauf
                                             -- (strava.ts), unabhängig vom Ergebnis; holt best_efforts für die
                                             -- letzten 20 Läufe gezielt nach, unabhängig vom splits_metric_json-
                                             -- Cache — sonst würde ein bereits bestehender Strava-Rekord nie
                                             -- entdeckt, da fetchActivityDetail() für schon analysierte
                                             -- Aktivitäten nie erneut läuft. Fire-and-forget in App.tsx neben
                                             -- syncPushSubscription() getriggert
push_opted_out        BOOLEAN DEFAULT false -- serverseitiges Push-Opt-out (statt nur localStorage); übersteht
                                             -- Logout (localStorage.clear()) und iOS "Icon entfernen + neu
                                             -- hinzufügen" (leert ebenfalls localStorage), wo
                                             -- Notification.permission trotzdem 'granted' bleibt
-- Persönliche Daten
...
```json
[
  { "type": "cycling",  "days": 2, "hours_per_week": 6 },
  { "type": "running",  "days": 2 },
...
- `hours_per_week`: optional, `number | null`, nur bei `cycling`/`running` in `Profile.tsx` abgefragt ("Ø Std./Woche"-Feld im Sportarten-Akkordeon) — steuert `determineTrainingPhilosophy()` (`coachContext.ts`, Polarisiert 80/20 vs. effizienzfokussiert) in der Wochenplan-Periodisierung, siehe Kapitel 10 „Plan-Generierung"
...
stimulus_checked     BOOLEAN DEFAULT NULL -- true nach erstem Stimulus-Check (triggerStimulusCheck), unabhängig
                                           -- vom Ergebnis; rein deterministisch, kein Claude-Call nötig — siehe
                                           -- Kapitel 9 „ActivityDetail.tsx" und Kapitel 10 „Plan-Generierung"
rpe                  INTEGER DEFAULT NULL -- CHECK 1-10, Rate of Perceived Exertion; nur Laufen, freiwillig vom
                                           -- Athleten in ActivityDetail.tsx eingetragen (kein Auto-Wert); dient
                                           -- als Veto für den stimulus_insufficient-Check, siehe Kapitel 10
is_interval          BOOLEAN DEFAULT NULL -- manuelle Intervalltraining-Markierung ("Intervalltraining?"-Toggle
                                           -- in ActivityDetail.tsx, nur Lauf/Rad), Fallback wenn Strava-Runden
                                           -- fehlen oder nur generische Auto-Splits sind — siehe Kapitel 9
                                           -- „ActivityDetail.tsx" (detectEffortSegments() in activityAnalysis.ts)
created_at      TIMESTAMPTZ
```
...
**Response:** `text/calendar; charset=utf-8` — ein VEVENT pro geplantem Trainingstag aus `weekly_plans` (neueste `version` je `week_start`), gefiltert auf die aktuelle Woche (Montag in Europe/Vienna, `currentWeekStartVienna()`) und Zukunft — vergangene Wochen erscheinen nicht im Feed. Ruhetage ausgelassen. Events starten um 18:30 Uhr Europe/Vienna (DST-sicher) für die geplante `duration_min` (Default 60 Min), mit `CATEGORIES:Sport` für Apple Kalenders Tag-Farbzuordnung. Titel: Kraft `💪🍑 Workout I/II/III`, Laufen `🏃 Laufen · Xmin`, Radfahren `🚴 Radfahren · Xmin`; Freitext-`description` bei Lauf/Rad steht in `DESCRIPTION`, nicht im Titel.  
...
- Filter-Buttons: WeightTraining / Ride / Run mit FA6-Icons (VirtualRide/VirtualRun werden mitgefiltert)
  - **Filter-Persistenz über Route-Unmount hinweg** (12. Juli 2026): `filter`-State wird zusätzlich in `sessionStorage` (`dashboard_filter`) gespiegelt (lazy `useState`-Initializer liest ihn beim Mount, jeder Klick auf einen Filter-Button schreibt/löscht ihn) — nötig, weil `<Routes>` `Dashboard` beim Navigieren zu `/activity/:id` komplett unmountet und der reine React-State beim Zurücknavigieren sonst auf `null` zurückgesetzt gewesen wäre, unabhängig davon ob per `navigate(-1)` oder hartem Redirect zurückgekehrt wird. Wird beim Logout durch das bestehende `sessionStorage.clear()` mit entfernt.
...
- **Konsistenz-Fix (21. Juli 2026):** Die drei `UPDATE activities`-Aufrufe innerhalb von `analyzeActivity()` (Cache-Schreiben von `streams_json`, `laps_json`, `description`) filterten bisher nur über `.eq('strava_id', ...)`. Unkritisch, da `strava_id` DB-weit `UNIQUE` ist, aber inkonsistent zu den athlete_id-gescopten Updates in `ActivityDetail.tsx`. Jetzt zusätzlich `.eq('athlete_id', athleteId)` — Defense-in-Depth, relevant geworden seit ein zweiter echter Athlet (eigenes Gerät, eigener Strava-Account) die App nutzt und die Isolation ausschließlich auf Anwendungsebene läuft (siehe Kapitel 18 „Multi-User Vorbereitung (RLS)")
...
- **Bugfix Kilometer-Splits im Analyse-Prompt (26. Juli 2026):** Der Kilometer-Splits-Abschnitt des `activityBlock`-Prompts in `analyzeActivity()` (`activityAnalysis.ts`) nutzte `elapsed_time` statt `moving_time` — bei einer Pause innerhalb eines Splits (z. B. GPS-Lock am Start) wich die von Claude im Analysetext genannte Zeit/Pace für diesen Kilometer von der Kilometer-Splits-Tabelle in `ActivityDetail.tsx` ab (z. B. Analysetext „km 1: 7:48" vs. Tabelle „7:24"), obwohl beide auf demselben `splits_metric_json` basieren. Jetzt konsistent `moving_time`, identisch zur UI-Tabelle (siehe „Kilometer-Splits-Tabelle" oben, `moving_time statt elapsed_time`).
- **Bugfix Dauer-Format ab 60 Minuten (26. Juli 2026):** Prompt-Text und UI zeigten die Dauer mehrstündiger Aktivitäten bisher als rohe Minutenzahl (`Math.round(duration_s / 60)`, z. B. „386 Minuten" im Wochenreview-Prompt statt „6 Std 26 Min"). Neuer gemeinsamer Helper `formatDurationHuman(seconds)` (`src/lib/dateUtils.ts`, `< 60 min` → `"X min"`, sonst `"H Std M Min"`) ersetzt die verstreuten Einzelstellen: `activityBlock`-Prompt (`activityAnalysis.ts`), Wochenreview-Prompt und zwei UI-Anzeigen (`WeeklyPlan.tsx`), Echtzeit-Alert-Konflikt-Prompt (`Dashboard.tsx`). Die Stopwatch-Formate (`formatDuration()` in `activityAnalysis.ts`/`Dashboard.tsx`, `H:MM:SS`) für Kilometer-Splits/Runden und die Aktivitätsliste bleiben unverändert, da dort kein Prosa-Text entsteht.
...
**Zurück-Button** (Bugfix 12. Juli 2026): Nutzt `navigate(-1)` (react-router) statt eines hart verdrahteten `<Link to="/dashboard">` — zeigt bei jedem Aufruf die tatsächlich vorherige Seite (z. B. Chat oder Wochenplan), nicht immer das Dashboard. Guard `window.history.state?.idx > 0` prüft vorher, ob überhaupt SPA-eigene History existiert; bei Deep-Links (direkter URL-Aufruf, Push-Benachrichtigung) ohne vorherige App-Navigation ist `idx` `0`/`undefined` → Fallback bleibt `navigate('/dashboard')` statt aus der App heraus zu navigieren.
...
- Übungskarten: Name, Muskelgruppe-Pill (aus 90+ Keyword-Lookup `MUSCLE_LABELS`/`primaryMuscleLabel()`, rein aus dem Übungsnamen geraten — Hevy liefert kein Muskelgruppen-Feld), Volumen-Pill, Set-Tags. Keywords decken sowohl englische als auch deutsche Hevy-Übungsnamen ab (16. September 2026 ergänzt, nachdem bei mehreren deutsch benannten Übungen — "Fliegende", "Vorgebeugtes Rudern", "Seitheben", "Einbeiniges Beckenheben", "Trizepsdrücken", "Clamshell", "Pallof Press" — keine Pille erschien, da die Liste zuvor nur englische Keywords enthielt)
...
**Empfehlung übernehmen (29. Juli 2026, nur Lauf/Rad):** Button „Empfehlung übernehmen" unterhalb der KI-Analyse, sichtbar wenn `claude_analysis` existiert UND `sportFromActivityType(activity.type)` `'running'` oder `'cycling'` liefert — Kraft ist bewusst ausgeschlossen, da Kraft-Tage im Plan immer den festen Namen „Workout I/II/III" tragen (siehe Kapitel 10 „Plan-Generierung"), in den eine Freitext-Empfehlung nicht passt.
- **Extraktion:** `extractPlanRecommendation({analysisText, sport})` (neu, `src/lib/planRecommendation.ts`) schickt den Analysetext + heutiges Datum/Wochentag per eigenem, schlankem `/api/analyse`-Call (kein `buildCoachSystemPrompt()`/`buildCoachContext()`, analog zu Roast Me) und lässt Claude die Empfehlung für die nächste Einheit strukturiert als JSON zurückgeben: `{day, duration_min, distance_km, intensity, description, reasoning}`. `day` ist der im Text genannte Wochentag (`"Do"` etc.) oder `null`, wenn keiner explizit genannt wird.
- **Zielwoche deterministisch berechnet:** `resolveTargetWeek(day)` vergleicht den ISO-Wochentag von `day` mit dem heutigen — liegt er bereits in der Vergangenheit dieser Woche oder ist heute, gilt die **nächste** Woche als Ziel, sonst die aktuelle; Claude wird die Wochenwahl bewusst nicht überlassen (Fehlerquelle).
- **Bestätigungsdialog:** zeigt `reasoning`, einen Wochen-Umschalter (Diese/Nächste Woche) und editierbare Felder (Dauer, bei Rad zusätzlich Distanz, Intensität, Beschreibung). Der Tag-Dropdown listet **ausschließlich** Tage, die im Zielplan bereits dieselbe Sportart tragen (`dayMatchesSport()`, `src/lib/weeklyPlan.ts`) — verhindert, dass die Übernahme die Trainingstag-Struktur des Plans verändert; ohne passenden Tag in der gewählten Woche erscheint ein Hinweis statt eines leeren/falschen Dropdowns.
- **Speicherung:** `applyPlanRecommendation()` lädt die neueste Plan-Version der Zielwoche (gleiche `weekStrFallback`-UTC-Behandlung wie `WeeklyPlan.tsx`), merged nur den gewählten Tag (`duration_min`/`distance_km`/`intensity`/`description`, `type` bleibt unverändert) und speichert über `insertPlanVersion()` (neu, `src/lib/weeklyPlan.ts` — INSERT-only, `version++`, extrahiert aus `WeeklyPlan.tsx`s `saveManualPlanChange()`, damit beide Wege dieselbe Speicherlogik nutzen) plus `coach_decisions`-Eintrag (`decision_type: 'plan_recommendation_applied'`, `related_activity_id` gesetzt, `related_plan_id` = neue Version).
- **Invariante gewahrt:** bei `sport === 'running'` bleibt `distance_km` immer `null`, auch wenn Claude einen Wert extrahiert hätte (siehe Kapitel 10 „Laufen zeigt nie `distance_km`").
- Verifiziert per Wegwerf-Testathlet (Playwright headless gegen `npm run dev`): Extraktion, Tag-Vorauswahl, Übernahme, neue Plan-Version mit korrekt gemergtem Tag, `coach_decisions`-Eintrag, alle übrigen Tage byte-identisch zur Vorversion; Testdaten anschließend gelöscht, echte Athleten-Zeilen unverändert.

**RPE-Erfassung (5. August 2026, nur Laufen):** Unterhalb der Charts/Splits-Tabelle, oberhalb der Kraft-Übungstabelle — Überschrift „Wie hart hat sich der Lauf angefühlt?" + Grid mit 10 Zahlen-Buttons (1–10). Sofort-Speicherung auf Tap (`saveRpe(value)`, `UPDATE activities SET rpe = value`), kein Modal/Submit-Schritt — die hervorgehobene Zahl selbst ist die Bestätigung; optimistisches State-Update, kein Warten auf den Roundtrip. Zweck: dient als Veto für den `stimulus_insufficient`-Check (siehe Kapitel 10) — eine hohe RPE (>6) bei einer als „zu leicht" geflaggten Einheit widerspricht dem reinen HF-Befund (der durch Hitze, Schlafmangel etc. täuschen kann), `WeeklyPlan.tsx`s Stimulus-Signale-Query filtert entsprechend über `.or('rpe.is.null,rpe.lte.6', {foreignTable: 'activities'})` und ignoriert das Signal bei hoher RPE, ohne den `coach_decisions`-Eintrag zu löschen.

**Intervalltraining-Markierung + Intervallstruktur-Erkennung (16. September 2026, Lauf + Rad):** Unterhalb der Charts/Splits-Tabelle ein Toggle-Button „Intervalltraining?" (`isRun || isCyclingForInterval` — Rad-Erkennung hier bewusst breiter als das bestehende `isRide`, schließt `MountainBikeRide`/`GravelRide` mit ein, deckt sich mit der Sportart-Erkennung in `activityAnalysis.ts`). Tap ruft `toggleIsInterval()` auf: `UPDATE activities SET is_interval = !is_interval`, danach sofort `runAnalysis(overrideActivity)` (neuer optionaler Parameter — der React-`activity`-State im Closure hätte direkt nach dem Toggle noch den alten Wert, State-Batching).

Grund: GPS-Uhren/Bike-Computer legen automatisch distanz-/zeitbasierte „Auto-Laps" an (meist exakt 1 km bzw. feste Zeitspanne), unabhängig davon, ob der Athlet manuell Runden gedrückt hat. `isGenericAutoSplit(laps)` (`src/lib/activityAnalysis.ts`) erkennt diese (alle Runden bis auf die letzte mit <5% Schwankung in Distanz ODER Dauer vom Mittelwert) und verwirft sie als für eine Intervallanalyse ungeeignet. Fehlen brauchbare Runden (keine oder nur generische Auto-Splits) UND der zugehörige Plantag gibt Z4/Z5 vor (`getPlannedDayZone()`, aus `triggerStimulusCheck()` extrahiert und jetzt auch von `analyzeActivity()` genutzt) ODER `activities.is_interval === true`, segmentiert `detectEffortSegments(data, sport)` den ohnehin vorhandenen Sekundenverlauf (Tempo für Laufen bzw. Watt für Rad + Puls) deterministisch in Belastungs-/Erholungsblöcke: Schwelle = Mitte zwischen 25.- und 75.-Perzentil des Effort-Werts DIESER Aktivität selbst (keine festen Zonen-Grenzen, passt sich automatisch ans Leistungsniveau an); Segmente <30s (`SEGMENT_NOISE_FLOOR_S`) werden als Rauschen ins Nachbarsegment eingeschmolzen, danach direkt benachbarte gleichartige Segmente zusammengefasst. `formatDetectedIntervals()` formatiert nur die Belastungssegmente als nummerierte „Intervall N"-Liste (Dauer, Pace/Watt, HF, nachfolgende Erholungsdauer) — Mindest-Wiederholungsdauer relativ zur beobachteten Median-Intervalldauer (`max(30s, median × 0.5)`) statt fixer Sekundengrenze, funktioniert dadurch für 400m- genauso wie 1km-Wiederholungen; unter 2 erkannten Wiederholungen: `null` (keine geratene Zusatzinfo). Ersetzt in diesem Fall im Analyse-Prompt die normale Runden-/Splits-Darstellung, die Analyse-Struktur-Anweisung wechselt entsprechend von „Rundenanalyse" zu „Intervallanalyse". Ersetzt den ursprünglichen Ansatz, Claude eine rohe Sekundentakt-Liste zum Selbst-Erkennen zu geben — der war in der Praxis zu ungenau (ein echtes 6×1km-Schwellentraining wurde daraus als „5 Blöcke à 7-15 Min" statt der tatsächlichen 6 Wiederholungen à ~5-6 Min gelesen).

Beim Testen zusätzlich aufgefallen und mitbehoben: der Coach nannte bei Empfehlungen für einen anderen Plantag als heute fälschlich „heute" statt den tatsächlichen Wochentag (z. B. einen für morgen geplanten Z2-Lauf als „heute" empfohlen, obwohl heute laut Plan ein Ruhetag war) — neuer Prompt-Abschnitt „EMPFEHLUNGEN MÜSSEN ZUM PLAN PASSEN" in `coachPrompt.ts` (siehe Kapitel 12) verlangt jetzt den Abgleich jeder Empfehlung mit dem `[AKTUELLER WOCHENPLAN]`-Kontext für den betreffenden Tag sowie die explizite Wochentag-Nennung, wenn sich eine Empfehlung nicht auf heute bezieht.
...
- Sportarten: Pills (Radfahren / Laufen / Krafttraining) mit Akkordeon-Stepper
  ...
  - **Ø Std./Woche (16. September 2026, nur Rad/Lauf):** optionales Number-Input im aufgeklappten Stepper-Bereich (`config.hours_per_week`), speichert direkt über `updateSportConfig()` in `sport_types`; leer = `null`, kein Pflichtfeld. Steuert `determineTrainingPhilosophy()` in der Wochenplan-Periodisierung (siehe Kapitel 10 „Sportspezifische Periodisierung"), sonst ohne weitere Wirkung
...
- Ziel-Liste: sortiert A → B → C, jede Ziel-Karte verlinkt auf `/goals/:id` (`GoalDetail.tsx`, siehe unten)
...
### GoalDetail.tsx (neu, 15. August 2026, Route `/goals/:id`)

Unterseite pro Saisonziel — zeigt (1) eine grafische Trainingsphasen-Anzeige und (2) für Laufziele mit hinterlegter Distanz eine geschätzte Zielzeit. **Komplett ohne Claude-Call** — reine Arithmetik auf echten `activities`/`strava_prs`/HF-Daten (`coachContext.ts`), damit die Anzeige nie „erfunden" wirkt.

**Phasen-Grafik:**
- 4-Segment-Balkengrafik (Readaptation/Grundlage/Wettkampfvorbereitung/Taper) aus `calculateSeasonPhase(weeksUntilEvent, override)` — derselben, bereits bestehenden, rein deterministischen Logik wie in `Profile.tsx`/`coachPrompt.ts` (siehe Kapitel 12), hier erstmals grafisch statt nur als Textlabel dargestellt
- Bereits durchlaufene Phasen: grün gefüllt + gelber Stern (`IconStar`); aktuelle Phase: anteilig gefüllt über `phase.progressPct` (0–100, aus der Position innerhalb der Wochen-Grenzen `PHASE_WEEK_BOUNDS` der Phase; `null` bei manuellem `season_phase_override`, da dort keine Zeitbasis existiert); kommende Phasen: grau
- Unter der Grafik: `phase.label` + `phase.description`, darunter entweder `calculatePhaseProgressNarrative()`s datenbasierter Satz (siehe Changelog-Kopf, z. B. „Dein Laufvolumen ist in den letzten 4 Wochen von 10 auf 19 km pro Woche gestiegen.") oder — falls die Mindestdatenmenge fehlt bzw. bei `taper` — der Fallback „X% dieser Phase geschafft" aus `progressPct`

**Zielzeit-Schätzung (nur `sport_type === 'Laufen'` mit gesetztem `distance_km`):**
- Zwei Methoden, versucht in dieser Reihenfolge (`GoalFinishEstimate`-Typ mit `estimatedSeconds`, `method`, `confidence`, `basisText`):
  1. **`estimateGoalFinishTimeFromEfficiency()`** (primär) — VO2max-Modell nach Daniels & Gilbert (1979) + ACSM %HFR≈%VO2R-Beziehung. `estimateVO2MaxFromRuns()` schätzt die aktuelle VO2max aus ≥3 durchgehenden Läufen ≥10 Min mit %HFR zwischen 30–90%, nimmt den Median der 3 höchsten Einzelschätzungen (robust gegen schlechte Tage/Sensor-Glitches). `predictSecondsFromVO2Max()` sucht per Bisektion die Zieldauer, bei der das nötige Tempo exakt die für diese Dauer laut Formel durchhaltbare VO2max ausschöpft (Standardverfahren öffentlicher VDOT-Rechner). Braucht **keinen** Maximaleinsatz — funktioniert auch aus bewusst ruhig gelaufenen Grundlagenläufen, löst damit die systematische Unterschätzung der reinen Distanz-Hochrechnung in der Grundlagenphase. `confidence`: `hoch` ab 6 Läufen + gemessener Max-HF, sonst `mittel`/`niedrig`.
  2. **`estimateGoalFinishTime()`** (Fallback, falls Methode 1 zu wenig Daten hat) — Riegel-Hochrechnung vom Kandidaten (Strava-PR aus `strava_prs` oder Trainingslauf ≥3 km), dessen Distanz der Zieldistanz am nächsten kommt (`gapRatio`, bei Gleichstand bevorzugt ein Strava-PR); `gapRatio > 6` → keine Schätzung (zu unzuverlässig). `confidence`: `hoch` bei `gapRatio ≤ 1.5`, `mittel` bis `3`, sonst `niedrig`.
- Beide Methoden liefern denselben `GoalFinishEstimate`-Typ, `GoalDetail.tsx` muss nicht wissen, welche gerade gegriffen hat. Klick auf den `basisText`-Link öffnet ein Overlay mit laienverständlicher Erklärung der Methode + 1-2 Quellenangaben (`METHOD_EXPLANATION`, statischer Text, kein Claude-Call: Daniels & Gilbert „Oxygen Power" bzw. Riegel „Athletic Records and Human Endurance")
- Voraussetzung für Methode 1: `saveStravaPrsIfPresent()`/`backfillStravaPrs()` (`strava.ts`) erfassen alle von Strava selbst erkannten Bestzeiten (`best_efforts`, Keys aus `STANDARD_DISTANCES_KM` in `coachContext.ts`: 1k/1 mile/2 mile/5k/10k/15k/10 mile/20k/half-marathon/30k/marathon) in `athletes.strava_prs` (siehe Kapitel 5)
- Ohne genug Daten für beide Methoden: Hinweistext „Noch nicht genug Trainingsdaten für eine verlässliche Schätzung."

**Entwicklung der Zielzeit — Progressions-Grafik (19. August 2026):** Da die Schätzung nie gespeichert wird (immer live berechnet), rechnet `calculateFinishTimeProgression()` (neu, `coachContext.ts`) dieselbe Methodenkette (Effizienz → Distanz-Anker) rückwirkend an `PROGRESSION_POINTS = 8` Zeitpunkten im `PROGRESSION_STEP_DAYS = 14`-Tage-Abstand neu (letzte 98 Tage) — für jeden Zeitpunkt NUR mit den Läufen/Strava-PRs, die zu diesem Zeitpunkt bereits vorlagen: eigenes 180-Tage-Fenster relativ zum jeweiligen Auswertungspunkt, `stravaPrs` nach ihrem `at`-Datum gefiltert. Ohne diese Rückdatierung würde ein späterer Formfortschritt fälschlich in frühere Punkte durchsickern. Die Aktivitäts-Query in `GoalDetail.tsx` holt entsprechend weiter zurück (`PROGRESSION_FETCH_DAYS = (PROGRESSION_POINTS-1) × PROGRESSION_STEP_DAYS + 180`, damit auch der älteste Auswertungspunkt sein eigenes 180-Tage-Fenster bekommt). HF-Profil (`effectiveMaxHR`/`restingHR`/`maxHrIsMeasured`) wird als aktuell für alle Zeitpunkte angenommen, da keine Historie davon existiert. Punkte ohne genug Daten werden ausgelassen statt als 0 geplottet (`FinishTimeProgressionPoint[]`, `{date, estimatedSeconds, method}`).

`GoalDetail.tsx` zeigt das Ergebnis als Recharts-`LineChart` ("Entwicklung der Zielzeit", nur ab ≥2 Punkten sichtbar): Y-Achse invertiert (`reversed`, oben = schneller), `tickFormatter`/Tooltip über `formatRaceTime()`. X-Achse zeigt das Datum (`DD.MM`), Tooltip-Label das volle Datum. Untertitel erklärt die rückwirkende Methode in Klartext.
...
- **Message-Anzeige (Bugfix 4. August 2026):** lädt die neuesten 50 Nachrichten je Thread — `order('created_at', {ascending: false}).limit(50)`, danach `.reverse()` für die chronologische Anzeige. Vorher `ascending: true` + `limit(50)`, was bei >50 Nachrichten im Thread dauerhaft die **ältesten** 50 zeigte statt der neuesten (Symptom: neue Nachrichten schienen nach dem Senden nicht anzukommen)
- Supabase-first Flow:
  1. User-Message → INSERT in `chat_messages`
  2. Reload aus DB
  3. `buildCoachContext(athleteId, athleteId)` aufrufen — Chat-Sektion selbst berücksichtigt seit 4. August 2026 die letzten 20 statt 10 Nachrichten (siehe Kapitel 11)
...
**"In Plan übernehmen" (2. August 2026):** Button unter der letzten Coach-Nachricht, analog zum bestehenden „Empfehlung übernehmen"-Button in `ActivityDetail.tsx` (siehe Kapitel 9 oben). `extractChatPlanRecommendation({chatText})` (`src/lib/planRecommendation.ts`) extrahiert Tag/Dauer/Distanz/Intensität/Beschreibung **und** zusätzlich die Sportart selbst strukturiert per eigenem `/api/analyse`-Call — anders als bei der Aktivitäts-Analyse ist die Sportart hier vorher nicht bekannt. Liefert Claude `sport: null` (Krafttraining oder unklare Nachricht ohne konkrete Lauf/Rad-Einheit), bricht die Übernahme mit Fehlermeldung ab — gleiche Kraft-Invariante wie beim Aktivitäts-Analyse-Pfad (Kraft-Tage tragen nur feste „Workout I/II/III"-Namen). `applyPlanRecommendation()` (`src/lib/weeklyPlan.ts`/`planRecommendation.ts`) wurde dafür generalisiert: `activityName`/`activityId` → optionales `source`-Freitextlabel (z. B. `"Chat-Empfehlung"` vs. `Analyse von "<Aktivitätsname>"`), `activityId` bleibt optional (aus dem Chat heraus gibt es keine Aktivität) — von beiden Einstiegspunkten (`ActivityDetail.tsx` und `Chat.tsx`) genutzt. Speicherung weiterhin INSERT-only als neue Plan-Version + `coach_decisions`-Eintrag (`decision_type: 'plan_recommendation_applied'`).

**Kein falsches "Plan gespeichert" (Bugfix 2. August 2026):** `Chat.tsx` schreibt technisch nie in `weekly_plans` (nur `chat_messages`) — auf Bitten wie „übernimm das in den Plan" bestätigte der Coach vor diesem Fix trotzdem mit „Plan aktualisiert ✅", obwohl real nichts gespeichert wurde. Neuer Prompt-Abschnitt „WOCHENPLAN-ÄNDERUNGEN IM CHAT" in `coachPrompt.ts` (`buildCoachSystemPrompt()`, gilt global für alle Claude-Calls inkl. Chat) verpflichtet den Coach, die Empfehlung stattdessen konkret zu formulieren und auf den „In Plan übernehmen"-Button zu verweisen — siehe Kapitel 12.
...
- `buildCoachContext(athleteId, athlete.id)` + `buildCoachSystemPrompt(athlete.id)` + `coach_decisions[type=recovery_required, letzte 7 Tage]` + `coach_decisions[type=stimulus_insufficient, letzte 14 Tage]` → parallel
- Woche (Montag-Datum als Referenz)
- `athlete.training_days_per_week` und `athlete.sport_types`

**Chat-Kontext (Bugfix 2. August 2026):** `buildCoachContext()` wurde hier bisher **ohne** `threadId` aufgerufen, wodurch Abschnitt „AKTUELLE CHAT-SESSION" des Kontexts immer leer blieb — Wünsche/Absprachen aus dem Coach-Chat hatten keinen Einfluss auf den generierten Plan. `generatePlan()`/`startReview()` übergeben jetzt beide `threadId = athlete.id` mit; kein hartes Erzwingen, Claude gewichtet den Chat-Kontext nur als ein Signal von mehreren.

**Stimulus-Signale (5. August 2026):** Zusätzlich zum bestehenden `recovery_required`-Query (7-Tage-Fenster) wird ein zweiter Query auf `coach_decisions[decision_type='stimulus_insufficient']` mit 14-Tage-Fenster geladen (etwas länger als Verletzungs-Flags, da Progressions-Signale etwas mehr nachwirken dürfen) und gefiltert über `.or('rpe.is.null,rpe.lte.6', {foreignTable: 'activities'})` — eine hohe RPE (siehe Kapitel 9 „ActivityDetail.tsx") widerspricht dem HF-Befund und schließt das Signal aus dem Prompt aus, ohne die `coach_decisions`-Zeile zu löschen. Nicht ausgeschlossene Signale fließen als neuer Prompt-Abschnitt „STIMULUS-SIGNALE" (MÜSSEN in der Pace-/Intensitätsvorgabe berücksichtigt werden, überschreiben nicht die harten Tage-Constraints) ein — spiegelt den bestehenden `recovery_required`-Mechanismus.

**Trainingsphase-Badge (16. August 2026):** Über der Claude-Zusammenfassung zeigt `WeeklyPlan.tsx` jetzt live via `calculateSeasonPhase()` berechnet, welche Phase gerade gilt (dasselbe primäre A-Ziel-Query wie `GoalDetail.tsx`/`coachPrompt.ts`, siehe Kapitel 9 „GoalDetail.tsx" und Kapitel 12) — statt sich auf Claudes freien `summary`-Text zu verlassen, der bei Plan-Generierung eingefroren wird und dabei sogar eine erfundene „Phase X Woche Y"-Aussage enthielt (kein Wochenzähler existiert im Code). Der `summary`-Prompt unten verbietet Claude seitdem explizit, Phase/Wochenzahl zu nennen; vor diesem Fix generierte Pläne behalten ihren alten Summary-Text bis zur nächsten Neugenerierung.

**Saisonziel-Woche hart abgesichert (13. September 2026):** Zwei reale Fehlplanungen behoben.
1. `checkGoalPlacement(planJson, monday, goals)` (`WeeklyPlan.tsx`) ist eine harte, Claude-unabhängige Nachkontrolle: prüft nach der Generierung rein anhand von `season_goals.event_date` (nicht Claudes eigener Datumsangabe im Fließtext), ob ein im Plan erwähntes Saisonziel am richtigen Wochentag steht — liegt das Event in der angezeigten Woche, muss der Tag, an dem der Eventname im Plan auftaucht, exakt `dayLabelForDate(eventDate)` entsprechen; taucht der Name an einem Tag außerhalb der richtigen Woche auf, ebenfalls ein Fehler. Läuft zusätzlich zu `validateConstraints()`, vor dem Speichern (`issues = [...validateConstraints(...), ...checkGoalPlacement(...)]`) — beide Fehlerlisten landen gemeinsam im Violation-Banner. Anlass: ein Wettkampf ("Tiroler Firmenlauf") wurde trotz korrekt genanntem Datum eine Woche zu früh eingetragen, weil Claude Wochentag↔Datum selbst falsch verrechnet hatte.
2. Neuer Prompt-Abschnitt `goalWeekSection` berechnet fest (nicht von Claude), ob/an welchem Wochentag ein aktives Saisonziel in die aktuelle Planwoche fällt: „WETTKAMPF IN DIESER WOCHE (fest berechnet, nicht selbst nachrechnen): — '{event_name}' ist am {Wochentag} ({event_date}) — trage an GENAU diesem Wochentag den Wettkampf ein" bzw. bei keinem Treffer explizit „KEIN Wettkampf/Saisonziel in dieser Woche … plane KEINE Renn-/Wettkampfeinheit, auch wenn ein Ziel im Kontext erwähnt wird."
3. Wochenreview (`startReview()`/`saveReviewData()`, siehe „Wochenreview" unten) liefert jetzt zusätzlich zum Review-Text ein `constraint`-Feld im JSON (`ReviewJson.constraint: string | null`) — Claude bewertet explizit, ob ein aktives körperliches Problem (Schmerzen, Verletzung, Beschwerden aus Aktivitäten ODER Freitext-Feedback) für die kommende Woche zwingend zu beachten ist. Ist es gesetzt, landet es als eigener `coach_decisions`-Eintrag (`decision_type: 'review_constraint'`, kein `related_activity_id` — Fenster über `created_at` statt Aktivitätsdatum, da kein Aktivitätsbezug) in derselben `recoveryLines`-Höchste-Priorität-Regel wie automatisch erkannte `recovery_required`-Signale (kombiniert in `generatePlan()`) — vorher flossen Review-Einschränkungen nur unpriorisiert im `review_notes`-Fließtext ein und konnten bei der nächsten Plan-Generierung untergehen.

**Sportspezifische Periodisierung — Rad/Lauf nach Trainingsphilosophie (16. September 2026):** Anlass: ein Athlet bereitet sich ab Ende Oktober auf ein Rad-Event im April vor und möchte, dass der Plan nach validierten sportwissenschaftlichen Prinzipien statt rein freiem Claude-Ermessen läuft. Greift NUR, wenn das primäre A-Ziel (`priority='A'`, frühestes `event_date` unter den aktiven Zielen) `sport_type` „Radfahren" oder „Laufen" ist — bei einem Kraft-Ziel, einer anderen Sportart oder ganz ohne A-Ziel bleibt es beim bisherigen generischen Regelwerk (z. B. keine Rad-spezifische Erzwingung, wenn Rad nur unterstützende Nebensportart zu einem Laufziel ist).

- `calculateSeasonPhase(weeksUntilEvent, override, sportType?)` (`coachContext.ts`) bekommt einen dritten, optionalen Parameter `sportType` (der `season_goals.sport_type`-Wert). Bei `"Radfahren"` liefert eine neue `PHASE_LABELS_CYCLING`-Map die Phasentexte statt der ursprünglich rein für Laufen geschriebenen `PHASE_LABELS` — dieselben 4 Phasen-Keys/Zeitgrenzen (`PHASE_WEEK_BOUNDS` bleibt sportartunabhängig identisch), aber ohne die Lauf-Wortwahl ("Sehnen, Gelenke und Laufmuskulatur readaptieren"): Phase 1 „Grundlage" (aerobe Basis aufbauen, überwiegend Z2), Phase 2 „Aufbau" (Schwellen-/VO2max-Intervalle einführen), Phase 3 „Wettkampfvorbereitung" (renn-spezifische Belastung), Phase 4 „Taper". Alle 6 Aufrufstellen geben jetzt den Sporttyp des jeweiligen A-Ziels mit: `coachPrompt.ts` (`buildCoachSystemPrompt()`, dafür `sport_type` zusätzlich in der A-Ziel-Query selektiert), `WeeklyPlan.tsx` (Phasen-Badge, `season_goals`-Query um `sport_type` erweitert), `GoalDetail.tsx` (2×, `goal.sport_type` war dort ohnehin schon geladen), `Profile.tsx` (2×, neuer State `primaryEventSport` aus einer um `sport_type` erweiterten Query).
- Neue harte Regel „SPORTSPEZIFISCHE PERIODISIERUNG" in `generatePlan()`s Prompt gibt phasenabhängig vor, welcher Intervalltyp diese Woche für die A-Ziel-Sportart angemessen ist — orientiert an etablierten Prinzipien (Coggan/Allen-Leistungszonen fürs Rad, Daniels/Pfitzinger-Tempoarbeit fürs Laufen, Seiler-Polarisierungsforschung für den 80/20-Zweig), bewusst OHNE harte Prozent-Zonen-Vorgabe pro Woche (z. B. „70% Z2") — das wäre ohne Streckenprofil/Wattmessung pro Einheit zu starr; stattdessen eine strukturelle Vorgabe analog zum bestehenden Detailgrad der Kraft-Rotations-Regel. Beispiel Rad: Grundlage „Überwiegend Z2-Grundlagenfahrten, Volumen aufbauen. Keine VO2max- oder anaeroben Intervalle (Z5)"; Aufbau „1–2× Schwellen- oder VO2max-Intervalltraining pro Woche einbauen, Rest weiterhin Z2"; Wettkampfvorbereitung „Renn-spezifische Intervalle (Streckenprofil beachten, z. B. Kletterintervalle bei Höhenmeter-Zielen)". Analoge Regeln fürs Laufen mit Tempo-/Schwellenläufen statt Sweet-Spot/VO2max-Terminologie, unter Beibehaltung der 10%-Regel. Landet auch als eigener Punkt im SELF-CHECK vor der JSON-Ausgabe (nur wenn `primaryGoalSportKey` gesetzt ist).
- Neue Funktion `determineTrainingPhilosophy(sportType: 'cycling'|'running', hoursPerWeek: number|null)` (`coachContext.ts`) wählt anhand der im Profil hinterlegten Wochenstunden (`SportConfig.hours_per_week`, neues optionales Feld, siehe Kapitel 5 „athletes" und Kapitel 9 „Profile.tsx") zwischen „Polarisiert (80/20)" (~80% Z1/Z2, ~20% Z4/Z5, Z3 minimiert) und einem effizienzfokussierten Ansatz — „Sweet-Spot-Base" beim Rad (gezielte Sweet-Spot-/Tempo-Blöcke im oberen Z3-Bereich statt hohem Grundlagenvolumen), „Schwellen-fokussiert" beim Laufen (mehr Tempo-/Schwellenläufe bei begrenztem Wochenumfang). Schwellenwert Rad **8h/Woche**, Laufen **5h/Woche** — niedriger angesetzt wegen höherer Gelenk-/Sehnenbelastung pro Stunde ohne den Impact-Vorteil des Rads. Ohne hinterlegte Stundenzahl: `null`, keine Philosophie-Zeile im Prompt (bewusst kein Raten aus fehlender Information).
- Im selben Zug Taper-Regel in `COACHING-PRINZIPIEN` (`coachPrompt.ts`, Punkt 8) korrigiert: vorher fix „-30% Volumen Woche 13, -50% Woche 14" — eine absolute Wochennummer, ersichtlich für einen ~14-wöchigen Block gedacht (typisch für einen Marathon-Zyklus) und ohne Bezug zur tatsächlichen, mechanisch berechneten Taper-Phase (`weeksUntilEvent < 2`). Für einen 24-26-wöchigen Rad-Vorbereitungsblock (wie im Auslöse-Fall) hätte diese absolute Zahl nie gegriffen. Jetzt relativ zum echten A-Event-Datum formuliert: „in den letzten 2 Wochen vor dem A-Event Volumen reduzieren (vorletzte Woche ca. -30%, letzte Woche ca. -50%) — Intensität bleibt erhalten. Diese Reduktion bezieht sich auf die tatsächlichen Wochen vor dem im Kontext genannten Event-Datum, nicht auf eine feste Wochennummer im Trainingsplan." Gilt sportartübergreifend (betraf vorher auch Lauf-Pläne mit untypischer Blocklänge).
...
WETTKAMPF IN DIESER WOCHE (fest berechnet, nicht selbst nachrechnen): / KEIN Wettkampf …
                                       ← goalWeekSection, siehe „Saisonziel-Woche hart abgesichert" oben
...
- {date}: {reasoning}                 ← aus coach_decisions, type='recovery_required' UND 'review_constraint'

STIMULUS-SIGNALE (müssen in Pace/Intensität berücksichtigt werden): ...

SPORTSPEZIFISCHE PERIODISIERUNG ({Radfahren|Laufen}, A-Ziel "{event_name}", aktuelle Phase: {phase.label}):
- {phasenabhängige Intervall-Vorgabe}
- Trainingsphilosophie ({hoursPerWeek}h/Woche laut Profil): {Polarisiert|Sweet-Spot-Base|Schwellen-fokussiert} — …
                                       ← nur wenn primäres A-Ziel Rad oder Lauf ist, siehe oben
...
**Tages-Key-Normalisierung (20.9.2026):** `planJsonWithDates()` zeigt Claude den aktuellen Plan mit Datum im Key (`"Sa 26.9.2026"`). Claude übernahm dieses Format in die eigene Plan-Ausgabe, wodurch die UI keine Tageskarten mehr fand (leere Karten) und `checkGoalPlacement()` fälschlich „steht am Sa 26.9.2026, gehört aber an Sa" meldete. `normalizePlanDayKeys()` (`weeklyPlan.ts`) reduziert die Keys direkt nach dem Parsen auf `Mo`–`So`; eingebaut in beide `parsePlanJson()`-Kopien (`WeeklyPlan.tsx`, `Dashboard.tsx`).
...
- Hilfsfunktionen in `coachContext.ts` (exportiert):
  - `calculateSeasonPhase(weeksUntilEvent, override, sportType?)` — Phase aus Wochen-Countdown oder manuellem Override. Optionaler dritter Parameter `sportType` (`season_goals.sport_type` des A-Ziels, z. B. `"Radfahren"`, 16. September 2026): wählt zwischen `PHASE_LABELS` (ursprünglich rein für Laufen formuliert, "Readaptation"/"Grundlagenaufbau"/"Wettkampfvorbereitung"/"Taper") und `PHASE_LABELS_CYCLING` (dieselben 4 Phasen-Keys/Zeitgrenzen, aber Rad-Wortlaut: "Grundlage"/"Aufbau"/"Wettkampfvorbereitung"/"Taper") — reine Textwahl, die Phasengrenzen selbst (`PHASE_WEEK_BOUNDS`) bleiben sportartunabhängig identisch
  - `determineTrainingPhilosophy(sportType: 'cycling'|'running', hoursPerWeek: number|null)` (16. September 2026) — wählt aus `SportConfig.hours_per_week` (Profil, siehe Kapitel 9 „Profile.tsx") zwischen "Polarisiert (80/20)" und einem effizienzfokussierten Ansatz ("Sweet-Spot-Base" Rad / "Schwellen-fokussiert" Lauf); Schwellenwert Rad 8h, Lauf 5h; `null` ohne hinterlegte Stundenzahl. Nur für die Wochenplan-Periodisierung genutzt (`WeeklyPlan.tsx`), nicht Teil dieses System-Prompts — siehe Kapitel 10 „Sportspezifische Periodisierung"
  ...
  - `calculatePhaseProgressNarrative(phaseKey, runningActivities, effectiveMaxHR, restingHR)` (15. August 2026) — datenbasierter Fortschrittssatz **innerhalb** der aktuellen Phase (nicht: welche Phase — das bleibt exklusiv `calculateSeasonPhase()`), nur für die Ziel-Detailseite (`GoalDetail.tsx`) genutzt, nicht Teil des Claude-Prompts; siehe Kapitel 9 „GoalDetail.tsx" für die volle Herleitung je Phase
- **5k-Pace-Referenz-Priorität (15. August 2026):** `best_5k_seconds` (manuelle Profil-PB) > `strava_prs['5k']` (von Strava selbst erkannte Bestzeit, siehe Kapitel 5 „athletes") > `estimateBest5kFromActivities()` (Riegel-Schätzung aus Trainingsläufen) als letzter Fallback
...
**`## WOCHENPLAN-ÄNDERUNGEN IM CHAT`** (statischer Prompt-Abschnitt, 2. August 2026): Verpflichtet den Coach, im Chat niemals zu behaupten, er habe den Wochenplan bereits gespeichert/aktualisiert — er hat dafür technisch keinen Zugriff (`Chat.tsx` schreibt nur in `chat_messages`). Stattdessen soll er die Empfehlung konkret formulieren und auf den „In Plan übernehmen"-Button verweisen (siehe Kapitel 9 „Chat.tsx"). Behoben nach einem Live-Bug, bei dem der Coach auf „übernimm das in den Plan" mit „Plan aktualisiert ✅" antwortete, ohne dass real etwas gespeichert wurde.

**`## EMPFEHLUNGEN MÜSSEN ZUM PLAN PASSEN`** (statischer Prompt-Abschnitt, 16. September 2026): Verlangt, dass jede Coach-Empfehlung (Aktivitäts-Analyse UND Chat) mit dem tatsächlichen Eintrag im `[AKTUELLER WOCHENPLAN]`-Kontext für den betreffenden Tag abgeglichen wird — steht dort ein Ruhetag, darf keine Trainingseinheit erfunden werden, stattdessen die Ruhe bestätigen oder explizit eine Planänderung vorschlagen. Jeder Plantag ist im Kontext mit Wochentag UND Datum benannt (z. B. „Do 17.9.2026"); bezieht sich eine Empfehlung auf einen anderen Tag als heute, muss IMMER dessen echter Wochentag genannt werden, nie „heute". Behoben nach einem Live-Bug: ein für morgen geplanter Z2-Lauf wurde als „heute" empfohlen, obwohl heute laut Plan ein Ruhetag war — Ursache war das Fehlen einer expliziten Verknüpfung zwischen "welcher Tag wird gerade wiedergegeben" und dem im Kontext ohnehin vorhandenen Datumsbezug.

**Taper-Regel korrigiert (16. September 2026):** `COACHING-PRINZIPIEN` Punkt 8 lautete vorher fix „-30% Volumen Woche 13, -50% Woche 14" — eine absolute Wochennummer ohne Bezug zum echten Event-Datum, ersichtlich für einen ~14-wöchigen Block (Marathon-Zyklus) gedacht und inkonsistent mit der mechanisch berechneten Taper-Phase (`weeksUntilEvent < 2` in `calculateSeasonPhase()`). Jetzt relativ zum tatsächlichen A-Event-Datum formuliert (letzte 2 Wochen vor dem Event, vorletzte ca. -30%, letzte ca. -50%) — gilt sportartübergreifend, ausgelöst durch einen Rad-Vorbereitungsblock mit ~24-26 Wochen Vorlaufzeit, für den die alte fixe Wochenzahl nie gegriffen hätte.
...
**Login lokal (Dev-Login):** Strava leitet nach dem OAuth-Login immer auf `VITE_STRAVA_REDIRECT_URI` (Live-Adresse) um. `Home.tsx` rendert deshalb nur bei `import.meta.env.DEV` pro Athlet einen Button „Dev-Login (ohne Strava)". `listDevAthletes()`/`devLoginAs()` (`strava.ts`) setzen `athlete_strava_id` in localStorage/sessionStorage, das `pf_athlete_id`-Cookie und den RLS-Kontext (`set_athlete_context`) direkt aus Supabase. Gleiche Datenbank wie live — lokale Tests schreiben echte Daten.

**Weiterleitung ohne lokale Schlüssel:** Fehlen `ANTHROPIC_API_KEY` bzw. `STRAVA_CLIENT_SECRET` in der `.env`, ruft die Dev-Middleware (`vite.config.ts`, `forwardToLive()`) die Live-Funktionen unter `peakform-wheat.vercel.app` auf (`/api/analyse`, `/api/strava-token`). Claude-Calls aus lokalen Tests laufen dann über den Live-Zugang. Sind die Schlüssel gesetzt, arbeitet die Middleware wie bisher direkt.
...
- **Intervalltraining-Markierung + automatische Intervallstruktur-Erkennung** (16. September 2026, Lauf + Rad): Toggle „Intervalltraining?" + `detectEffortSegments()`-Fallback für Aktivitäten ohne brauchbare Rundendaten, siehe Kapitel 9 „Intervalltraining-Markierung + Intervallstruktur-Erkennung"
...
- **Ø Std./Woche pro Sportart** (16. September 2026, nur Rad/Lauf): optionales Feld in `sport_types` (`hours_per_week`), steuert die Trainingsphilosophie in der Wochenplan-Periodisierung
...
- **Saisonziel-Woche hart abgesichert** (13. September 2026): `checkGoalPlacement()`-Nachkontrolle + fest berechnete `goalWeekSection` im Prompt; Wochenreview liefert zusätzlich ein `constraint`-Feld, das als `review_constraint`-Decision in die Erholungs-Höchste-Priorität-Regel einfließt, siehe Kapitel 10 „Saisonziel-Woche hart abgesichert"
- **Sportspezifische Periodisierung Rad/Lauf** (16. September 2026): phasenabhängige Intervall-Vorgabe + Trainingsphilosophie (Polarisiert 80/20 vs. effizienzfokussiert) aus `SportConfig.hours_per_week`, nur wenn das primäre A-Ziel Rad oder Lauf ist; Taper-Regel im selben Zug auf das echte Event-Datum statt einer fixen Wochennummer umgestellt, siehe Kapitel 10 „Sportspezifische Periodisierung"
...
- Dedupliziert `weekly_plans` auf die neueste `version` je `week_start`; filtert Wochen vor der aktuellen Woche heraus (`currentWeekStartVienna()`, Montag der laufenden Woche in Europe/Vienna, Bugfix 27. Juli 2026 — vorher enthielt der Feed auch alle Vergangenheits-Wochen); Ruhetage werden ausgelassen; stabile UID pro Tag (kein Duplikat bei wiederholtem Abo-Polling)
...
**Coach-Empfehlungen gezielt in den Plan übernehmen (29. Juli – 2. August 2026):** Button „Empfehlung übernehmen" in `ActivityDetail.tsx` (nur Lauf/Rad) und „In Plan übernehmen" im Chat (`Chat.tsx`) — beide extrahieren strukturiert per eigenem `/api/analyse`-Call, berechnen die Zielwoche deterministisch statt Claude zu überlassen, und speichern über die geteilte `applyPlanRecommendation()`/`insertPlanVersion()`-Logik INSERT-only mit `coach_decisions`-Audit-Eintrag. Siehe Kapitel 9 „ActivityDetail.tsx — Empfehlung übernehmen" und „Chat.tsx".

**Trainingssteuerung aus echten Daten statt nur Kalender (5.–16. August 2026):**
- RPE-Erfassung nach Laufeinheiten (1-10-Picker, `ActivityDetail.tsx`) als Veto für den Stimulus-Check
- Deterministischer Stimulus-Check `triggerStimulusCheck()`: Ist-HF strukturell unter der geplanten Z2/Z3-Zielzone → `coach_decisions`-Signal, fließt als harte Regel in die nächste Plan-Generierung ein (kein Claude-Call)
- Chat-Historie fließt jetzt korrekt in Plan-Generierung/-Review ein (`threadId`-Fix); Coach behauptet im Chat nicht mehr fälschlich, den Plan gespeichert zu haben
- Ziel-Detailseite (`/goals/:id`, `GoalDetail.tsx`): Trainingsphasen-Grafik + datenbasierte Fortschrittsbeschreibung + Claude-freie Zielzeit-Schätzung (VO2max-Effizienz-Modell primär, Riegel-Distanz-Anker als Fallback) + rückwirkende Progressions-Grafik der Zielzeit über die letzten 98 Tage (`calculateFinishTimeProgression()`, 19. August 2026); Wochenplan zeigt dieselbe live berechnete Phase jetzt ebenfalls als eigenes Badge statt Claudes eingefrorenem Freitext
- Von Strava selbst erkannte Bestzeiten (`strava_prs`, alle Standarddistanzen) als zusätzliche Pace-Referenz-Quelle
- Siehe Kapitel 9 „ActivityDetail.tsx", „GoalDetail.tsx", „Chat.tsx", Kapitel 10 „Plan-Generierung" und Kapitel 12
...
- `useVisibleTabs.ts`: lädt features aus Supabase (einmalig per mount) und filtert Tabs; genutzt von `BottomNav.tsx` (Rendering) und `useTabSwipeNavigation.ts` (Swipe-Ziel) als gemeinsame Quelle der Wahrheit
...
#### Bugfix 16. August 2026 — Plan-Summary nannte eine eingefrorene, teils erfundene Trainingsphase

**Symptom:** Der Wochenplan zeigte im Summary-Banner „Phase 1 Woche 7: …", während die Ziel-Detailseite (`GoalDetail.tsx`, live berechnet) zur selben Zeit „Phase 2 — Grundlagenaufbau" auswies — ein für den Nutzer nicht auflösbarer Widerspruch, da beide Anzeigen scheinbar dieselbe Information zeigen sollten.

**Root Cause:** `plan_json.summary` (Schema-Vorgabe in **sowohl** `generatePlan()` (`WeeklyPlan.tsx`) **als auch** dem „Plan anpassen"-Call oben in diesem Kapitel) wird von Claude einmalig bei der Generierung frei formuliert und danach unverändert in der DB gespeichert. Claude bekommt zwar `phase.label` als Kontext-Info im System-Prompt (siehe Kapitel 12), war aber nicht daran gehindert, sich das zusätzlich in eigenen Worten (inkl. einer erfundenen „Woche N"-Zählung, für die es im Code keinen Wert gibt) in den Summary-Freitext zu schreiben — der dann bei jedem künftigen Seitenaufruf unverändert angezeigt wird, auch wenn die live berechnete Phase (z. B. durch fortschreitende Zeit bis zum Event) inzwischen weitergerückt ist.

**Fix:** `WeeklyPlan.tsx` lädt jetzt selbst das primäre A-Ziel und zeigt die Phase live über `calculateSeasonPhase()` als eigenes Badge über dem Summary-Text (identische Query wie `coachPrompt.ts`/`GoalDetail.tsx`). Die `summary`-Feld-Instruktion in **beiden** betroffenen JSON-Schemas (`generatePlan()` und dem „Plan anpassen"-Call in `Dashboard.tsx`) verbietet Claude jetzt explizit, Phase oder Wochenzahl zu nennen. Vor dem Fix generierte/angepasste Pläne behalten ihren alten Summary-Text bis zur nächsten Neugenerierung (INSERT-only, kein rückwirkendes Update der `plan_json`-Historie).
