...
- Bei Klick: `getRoastAnalysis(activity, {name}, stats, exercises)` (lokal in `ActivityDetail.tsx`) ruft `/api/analyse` **direkt** auf (`fetch`, `max_tokens: 300`) — **kein** `buildCoachSystemPrompt()`, **kein** `buildCoachContext()`, **kein** `buildSpecialistContext()`. System-Prompt kommt ausschließlich aus `buildRoastPrompt({name, sport})`
...
- Ergebnis-Card im Flammen-Look: Gradient `from-red-950/40 to-orange-950/30`, orangener Rand, Header „🔥 Geröstet 🔥"
