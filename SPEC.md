...
**Roast Me (vereinfacht 3. Juli 2026, vormals „Spaß-Analyse" mit 3 Modi):**
...
- Ein einzelner Button „🔥 Roast Me 🔥" (Flammen-Icon links+rechts, Gradient orange→rot, weißer fetter Text) erscheint **nur wenn `claude_analysis` bereits existiert**, direkt unterhalb der ernsten KI-Analyse-Card
- Bei Klick: `getRoastAnalysis(activity, {name}, stats, exercises)` (lokal in `ActivityDetail.tsx`) ruft `/api/analyse` **direkt** auf (`fetch`, `max_tokens: 500`) — **kein** `buildCoachSystemPrompt()`, **kein** `buildCoachContext()`, **kein** `buildSpecialistContext()`. System-Prompt kommt ausschließlich aus `buildRoastPrompt({name, sport})`
