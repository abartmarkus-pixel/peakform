...
- **Wochen-Kennzahlen-Leiste** (3. Juli 2026): Card zwischen Phasen-Banner (`displayPlanJson.summary`) und DayCards — Zeile 1 „X / Y Einheiten" (X = an geplanten Trainingstagen als `completed` gematchte Tage, Y = geplante Trainingstage ohne Ruhetage), Zeile 2 drei Werte mit `SPORT_DISPLAY`-Icons/-Farben: Lauf-km + Rad-km (Summe `distance_m` über `weekActivities` gefiltert nach `SPORT_MATCH.running`/`.radfahren`-Typen), Kraft-Gesamtgewicht (Summe `parseHevyDescription(a.description).totalVolume` über alle `WeightTraining`/`Workout`-Aktivitäten der Woche)
  - Berechnung in `useMemo` (`weekStats`, Dependencies `[displayPlanJson, weekActivities, monday]`) — synchron, kein eigener Ladezustand nötig
  - Ausblend-Bedingung: nur wenn `weekActivities.length > 0 ODER completedCount > 0` (verhindert leere „0 / Y Einheiten"-Karte bei zukünftigen Wochen ohne jegliche Aktivität)
