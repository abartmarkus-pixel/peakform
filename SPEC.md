...
│       ├── funModePrompts.ts  # buildFunModePrompt(mode, {name, gender, sport}): string — Prompts für die Spaß-Analyse
│       │                        FunMode = 'sarcastic'|'roast'|'sexy'; SportFocus = 'running'|'cycling'|'strength'|null
...
│       │                        Sexy-Modus: explizit zweideutig ("unter der Gürtellinie"), Trainingsvokabular als
│       │                        Wortspiel-Vehikel — Grenze bleibt hart: kein explizites Beschreiben sexueller
│       │                        Handlungen, kein Kommentar zu Körper/Aussehen der Person
│       │                        FUN_MODE_LABELS; komplett unabhängig von coachPrompt.ts/coachContext.ts —
│       │                        keine gemeinsame Logik, kein DB-Zugriff
...
- Bei Klick: `getFunAnalysis(mode, activity, {name, gender}, stats, exercises)` (lokal in `ActivityDetail.tsx`) ruft `/api/analyse` **direkt** auf (`fetch`, `max_tokens: 300`) — **kein** `buildCoachSystemPrompt()`, **kein** `buildCoachContext()`, **kein** `buildSpecialistContext()`. System-Prompt kommt ausschließlich aus `buildFunModePrompt(mode, {name, gender, sport})`
...
- User-Prompt (`buildFunStatsText()`) ist ein sportart-spezifischer Rohdaten-Stats-Block dieser einen Aktivität — nicht mehr nur generisch: Laufen bekommt Ø-Pace (aus Distanz/Dauer berechnet via `speedToPace()`) + Ø-Kadenz in spm; Rad bekommt NP/Ø-Watt + Ø-Trittfrequenz in rpm; Kraft bekommt pro Übung Gewicht×Wiederholungen je Satz + Muskelgruppe (`primaryMuscleLabel()`) + Gesamtvolumen. Ohne erkannte Sportart (`sport === null`) bleibt es beim bisherigen generischen Block (Distanz, Ø/Max HF)
