...
**`saveReviewData()`:**
- INSERT in `weekly_plans` (nächste Woche, version++): `review_notes = data.review` (Claude-Bewertungstext) UND `review_user_input = reviewFeedback.trim() || null` (roher, unveränderter Freitext des Athleten aus der Textarea)
- INSERT in `coach_decisions`: `decision_type = 'weekly_review'`
- Setzt `nextWeekPlan` (React-State) auf den neu eingefügten Datensatz — dadurch greift Fall B (siehe unten) sofort, ohne Reload

**`review_notes`/`review_user_input` Semantik:** Die Bewertung der Woche W (Claude-Text + roher Nutzer-Freitext) wird auf `weekly_plans` der Woche W+1 gespeichert. `buildCoachContext()` liest `review_notes` beim Generieren eines neuen Plans; `review_user_input` ist reiner Anzeige-Zweck (Kapitel 10, „Wochenreview-Ergebnis-Karte"), fließt nicht in den Coach-Kontext ein.

---

### Wochenreview-Ergebnis-Karte (`WeeklyReviewCard`, 6. Juli 2026)

Aufklappbare Karte (Titel „Wochenreview" + Chevron, lokaler `expanded`-State, **immer** `true` bei Mount — kein persistierter Zustand, jedes Neuladen der Woche startet ausgeklappt, da die Komponente über `key={weekStr}`/`key={"week-"+weekStr}` bei Wochenwechsel neu gemountet wird). Inhalt: „Deine Notizen:" (nur gerendert wenn `userInput` vorhanden — Legacy-Reviews vor dem 6. Juli 2026 haben `review_user_input = null`) + „Coach-Bewertung:" (immer vorhanden).

Wird in zwei unabhängigen Fällen angezeigt, beide datenbankgetrieben (kein ephemerer State):

- **Fall A — Plan dieser Woche entstand aus einem Review:** Falls der geladene `plan`-Datensatz der angezeigten Woche selbst `review_notes` trägt (weil das Review der Vorwoche, das diesen Plan erzeugt hat, bereits durchgeführt wurde), erscheint die Karte direkt oberhalb des Plan-Inhalts (`plan-summary`), unterhalb der Wochen-Navigation. Ergänzt den Plan, ersetzt ihn nicht — DayCards bleiben normal sichtbar.
- **Fall B — Woche W wurde bereits reviewt:** Beim Laden einer Woche W wird zusätzlich `nextWeekPlan` geladen (Query auf `weekly_plans` mit `week_start = W+1`, gleiche UTC-Fallback-Logik wie beim Haupt-Plan-Query). Trägt dieser Datensatz `review_notes`, gehört das Review inhaltlich zu Woche W: Die Karte ersetzt in der „Wochenreview"-Sektion vollständig das bisherige Eingabe-Formular (Aktivitätsliste + Textarea + „Wochenreview starten"-Button). Ohne `nextWeekPlan.review_notes` (Woche W noch nicht reviewt) bleibt das Formular unverändert sichtbar.

Beide Fälle können auf derselben Seite gleichzeitig zutreffen (aktuelle Woche zeigt sowohl das Review der Vorwoche oben als auch — falls bereits erledigt — die Fall-B-Karte unten anstelle des Formulars).
