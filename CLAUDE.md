...
athletes (id uuid PK, strava_athlete_id bigint UNIQUE, strava_access_token text,
          ...
          sport_types jsonb,           ← SportConfig[] {type, days, hours_per_week?}; cycling/running/strength.
                                      -- hours_per_week (optional, nur bei cycling/running in Profile.tsx
                                      -- abgefragt) steuert die Trainingsphilosophie (Polarisiert 80/20 vs.
                                      -- effizienzfokussiert) in der Wochenplan-Periodisierung, siehe
                                      -- determineTrainingPhilosophy() in coachContext.ts
...
activities (id uuid PK, athlete_id uuid FK→athletes, strava_id bigint UNIQUE,
            ...
            is_interval bool,        ← manuelle Intervalltraining-Markierung ("Intervalltraining?"-Toggle in
                                      -- ActivityDetail.tsx, nur Lauf/Rad), Fallback wenn Strava-Runden fehlen
                                      -- oder nur generische Auto-Splits sind (isGenericAutoSplit()) — siehe
                                      -- detectEffortSegments() in activityAnalysis.ts
...
│   │   │                   # calculateSeasonPhase(weeksUntilEvent, override, sportType?) (exportiert) — sportType
│   │   │                   # (season_goals.sport_type, z.B. "Radfahren") wählt zwischen PHASE_LABELS (Lauf-Wortlaut)
│   │   │                   # und PHASE_LABELS_CYCLING (Grundlage/Aufbau/Wettkampfvorbereitung/Taper, Rad-Wortlaut)
│   │   │                   # calculateHRZones(), calculatePaceReference() (exportiert)
...
│   │   │                   # calculateFinishTimeProgression(): rechnet die Zielzeit-Schätzung rückwirkend an
│   │   │                   # mehreren Zeitpunkten neu (Progressions-Grafik in GoalDetail.tsx)
│   │   │                   # determineTrainingPhilosophy(sportType, hoursPerWeek): Polarisiert 80/20 vs.
│   │   │                   # effizienzfokussiert (Sweet-Spot-Base/Schwellen-fokussiert), aus SportConfig.hours_per_week
...
- [x] Intervallstruktur-Erkennung ohne brauchbare Rundendaten (Lauf + Rad): GPS-Uhren/Bike-Computer legen automatisch distanz-/zeitbasierte "Auto-Laps" an (meist exakt 1 km bzw. feste Zeitspanne) — `isGenericAutoSplit(laps)` (`activityAnalysis.ts`) erkennt diese (alle Runden bis auf die letzte haben nahezu identische Distanz ODER Dauer, Schwankung <5% vom Mittelwert) und verwirft sie als für eine Intervallanalyse ungeeignet. Fehlen brauchbare Runden (keine ODER nur generische Auto-Splits) UND der Plantag gibt Z4/Z5 vor ODER die Aktivität ist manuell als Intervalltraining markiert (`activities.is_interval`, Toggle "Intervalltraining?" in `ActivityDetail.tsx`, nur Lauf/Rad sichtbar — `toggleIsInterval()` speichert sofort und stößt automatisch eine Neu-Analyse an, da die automatische Analyse nach dem Sync meist schon vorher lief), segmentiert `detectEffortSegments(data, sport)` den ohnehin vorhandenen Sekundenverlauf (Tempo/Watt + Puls) deterministisch in Belastungs-/Erholungsblöcke — Schwelle ist die Mitte zwischen 25.- und 75.-Perzentil des Effort-Werts DIESER Aktivität selbst (keine festen Zonen-Grenzen, passt sich automatisch ans Leistungsniveau an), funktioniert dadurch gleichermaßen für Lauf und Rad sowie für unterschiedliche Intervalllängen. `formatDetectedIntervals()` gibt nur bei ≥2 erkannten Wiederholungen eine "Intervall N"-Liste aus (sonst `null`, keine geratene Zusatzinfo), Mindest-Wiederholungsdauer relativ zur beobachteten Median-Intervalldauer statt fixer Sekundengrenze. Ersetzt den ursprünglichen Ansatz, Claude eine rohe Sekundentakt-Liste zum Selbst-Erkennen zu geben — der war zu ungenau (ein echtes 6×1km-Schwellentraining wurde als "5 Blöcke à 7-15 Min" statt der tatsächlichen 6 Wiederholungen gelesen)
- [x] Coach-Empfehlungen müssen zum tatsächlichen Wochenplan passen: fester Prompt-Abschnitt "EMPFEHLUNGEN MÜSSEN ZUM PLAN PASSEN" (`coachPrompt.ts`, wirkt global für Aktivitäts-Analyse UND Chat) verlangt den Abgleich jeder Empfehlung mit dem `[AKTUELLER WOCHENPLAN]`-Kontext für den betreffenden Tag (kein Erfinden von Trainingsinhalten für einen dort eingetragenen Ruhetag) UND die explizite Wochentag-Nennung, wenn sich eine Empfehlung auf einen anderen Tag als heute bezieht (nie "heute" sagen für einen Planinhalt, der laut Kontext an einem anderen Tag steht) — realer Bug: ein für morgen geplanter Z2-Lauf wurde als "heute" empfohlen, obwohl heute laut Plan ein Ruhetag war
...
- [x] Ziel-Detailseite (`src/pages/GoalDetail.tsx`, Route `/goals/:id`, von einer Ziel-Karte in `Goals.tsx` verlinkt): zeigt (1) eine 4-Segment-Phasen-Grafik (Readaptation/Grundlage/Wettkampfvorbereitung/Taper, aktuelle Phase hervorgehoben) aus `calculateSeasonPhase()` — bereits bestehende, rein deterministische Logik, kein neuer Code nötig — und (2) für Laufziele mit hinterlegter Distanz eine geschätzte Zielzeit. Komplett ohne Claude-Call — reine Arithmetik auf echten `activities`/`strava_prs`/HF-Daten (`coachContext.ts`), damit die Schätzung nie "erfunden" wirkt. `calculateSeasonPhase()` gibt zusätzlich `progressPct` zurück (0–100, aus dem Wochen-Countdown innerhalb der Phasengrenzen `PHASE_WEEK_BOUNDS`; `null` bei manuellem `season_phase_override`, da dort keine Zeitbasis existiert) — bereits durchlaufene Phasen zeigt die Balkengrafik grün gefüllt mit gelbem Stern (`IconStar`) statt der Ausgangsfarbe. Zwei Methoden, `GoalDetail.tsx` versucht sie in dieser Reihenfolge:
  ...
  - **Entwicklung der Zielzeit (Grafik):** `calculateFinishTimeProgression()` (`coachContext.ts`) rechnet dieselbe Methodenkette (Effizienz → Distanz-Anker) rückwirkend an 8 Zeitpunkten im 14-Tage-Abstand (letzte 98 Tage) neu — jeweils NUR mit den Läufen/Strava-PRs, die zum jeweiligen Zeitpunkt bereits vorlagen (eigenes 180-Tage-Fenster pro Auswertungspunkt, PRs nach ihrem `at`-Datum gefiltert), damit kein späterer Formfortschritt fälschlich in frühere Punkte durchsickert; HF-Profil wird als aktuell für alle Zeitpunkte angenommen (keine Historie davon vorhanden). Punkte ohne genug Daten werden ausgelassen statt als 0 geplottet. `GoalDetail.tsx` zeigt das als Recharts-`LineChart` mit invertierter Y-Achse (oben = schneller), nur ab ≥2 Punkten sichtbar.
...
- [x] Sportarten-Akkordeon mit Stepper — Invariante: `Σdays ≤ trainingDays` technisch erzwungen
  ...
  - [x] Optionales Stundenfeld "Ø Std./Woche" bei Rad/Lauf (`SportConfig.hours_per_week`) — steuert die Trainingsphilosophie (Polarisiert vs. effizienzfokussiert) in der Wochenplan-Periodisierung, siehe Wochenplan-Sektion. Ohne Angabe wird nichts erzwungen (kein Raten aus fehlender Information)
...
- [x] Trainingsphase-Badge über der Plan-Zusammenfassung: live via `calculateSeasonPhase()` (dasselbe A-Ziel-Query wie `GoalDetail.tsx`/`coachPrompt.ts`) statt aus Claudes `plan_json.summary` — der Summary-Text wurde vorher von Claude frei formuliert (inkl. einer erfundenen "Phase X Woche Y"-Aussage ohne echten Wochenzähler dahinter) und beim Generieren in der DB eingefroren, lief also auseinander mit der live berechneten Phase auf der Ziel-Detailseite. Summary-Prompt (hier UND in Dashboard.tsx's Echtzeit-Konflikt-Anpassung) verbietet Claude seitdem explizit, Phase/Wochenzahl zu nennen — `calculateSeasonPhase()` bleibt die einzige Quelle der Wahrheit für die Phase, Claude bekommt sie nur als Kontext-Info, nie zur eigenen Ausgabe. Vor diesem Fix generierte Pläne behalten ihren alten Summary-Text bis zur nächsten Neugenerierung (INSERT-only, kein rückwirkendes Update)
- [x] Saisonziel-Woche hart abgesichert: `checkGoalPlacement(planJson, monday, goals)` (`WeeklyPlan.tsx`) prüft nach der Generierung, rein anhand von `event_date` (nicht Claudes eigener Datumsangabe im Fließtext), ob ein im Plan erwähntes Saisonziel am richtigen Wochentag steht — läuft zusätzlich zu `validateConstraints()`, vor dem Speichern (realer Vorfall: ein Wettkampf wurde trotz korrekt genanntem Datum eine Woche zu früh eingetragen, weil Claude Wochentag↔Datum selbst falsch verrechnet hatte). Ergänzend berechnet ein neuer Prompt-Abschnitt `goalWeekSection` fest (nicht von Claude), ob/an welchem Wochentag ein aktives Saisonziel in die aktuelle Planwoche fällt ("WETTKAMPF IN DIESER WOCHE" bzw. explizit "KEIN Wettkampf in dieser Woche — plane KEINE Renn-/Wettkampfeinheit"). Im Wochenreview liefert Claude zusätzlich ein `constraint`-Feld (aktives körperliches Problem oder `null`); ist es gesetzt, landet es als `coach_decisions`-Eintrag (`decision_type: 'review_constraint'`) in derselben Höchste-Priorität-Regel wie automatisch erkannte `recovery_required`-Signale — ein im Review genanntes Problem (z.B. Schmerzen) kann dadurch bei der nächsten Plan-Generierung nicht mehr untergehen
- [x] Sportspezifische Periodisierung (Rad/Lauf) nach Trainingsphilosophie: greift in `generatePlan()` (`WeeklyPlan.tsx`) NUR, wenn das primäre A-Ziel (höchste Priorität, frühestes Datum) `sport_type` "Radfahren" oder "Laufen" ist — bei einem Kraft-Ziel, einer anderen Sportart oder ohne A-Ziel bleibt es beim generischen Regelwerk (z.B. keine Rad-spezifische Erzwingung, wenn Rad nur unterstützende Nebensportart zu einem Laufziel ist). Neue harte Regel "SPORTSPEZIFISCHE PERIODISIERUNG" gibt phasenabhängig vor, welcher Intervalltyp diese Woche angemessen ist — an etablierten Periodisierungsprinzipien orientiert (Coggan/Allen-Leistungszonen fürs Rad, Daniels/Pfitzinger-Tempoarbeit fürs Laufen, Seiler-Polarisierungsforschung für den 80/20-Zweig), aber bewusst ohne harte Prozent-Zonen-Vorgabe pro Woche (z.B. "70% Z2") — das wäre ohne Streckenprofil/Wattmessung pro Einheit zu starr, stattdessen strukturelle Vorgabe analog zum bestehenden Detailgrad der anderen harten Plan-Regeln. Beispiel Rad: Grundlage "überwiegend Z2, keine VO2max/anaerobe Intervalle", Aufbau "1-2× Schwellen-/VO2max-Intervalltraining pro Woche", Wettkampfvorbereitung "renn-spezifische Intervalle, z.B. Kletterintervalle bei Höhenmeter-Zielen". Zusätzlich `determineTrainingPhilosophy(sportType, hoursPerWeek)` (`coachContext.ts`): wählt aus dem optionalen `SportConfig.hours_per_week` (Profil-Sektion) zwischen "Polarisiert (80/20)" und einem effizienzfokussierten Ansatz ("Sweet-Spot-Base" beim Rad, "Schwellen-fokussiert" beim Laufen) — Schwellenwert Rad 8h/Woche, Laufen 5h/Woche (niedriger angesetzt wegen höherer Gelenk-/Sehnenbelastung pro Stunde ohne den Impact-Vorteil des Rads). Ohne hinterlegte Stundenzahl: keine Philosophie-Vorgabe (kein Raten). Landet auch als eigener Punkt im Self-Check vor der JSON-Ausgabe. `calculateSeasonPhase()` bekommt dafür einen dritten, optionalen Parameter `sportType` (`season_goals.sport_type`) — bei `"Radfahren"` liefert eine neue `PHASE_LABELS_CYCLING`-Map die Phasentexte (Phase 1 "Grundlage" statt "Readaptation/Sehnen-Laufmuskulatur", Phase 2 "Aufbau" statt "Grundlagenaufbau/Kilometerzahl") statt der ursprünglich rein für Laufen geschriebenen Formulierungen — alle 6 Aufrufstellen (`coachPrompt.ts`, `WeeklyPlan.tsx`, `GoalDetail.tsx`, `Profile.tsx`) geben jetzt den Sporttyp des jeweiligen A-Ziels mit. Im selben Zug Taper-Regel in den `COACHING-PRINZIPIEN` (`coachPrompt.ts`, Punkt 8) korrigiert: vorher fix "-30% Volumen Woche 13, -50% Woche 14" (unpassend für alles außer ~14-Wochen-Blöcke, z.B. einen 24-26-wöchigen Rad-Vorbereitungsblock) — jetzt relativ zum echten A-Event-Datum formuliert (letzte 2 Wochen vor dem Event, vorletzte ca. -30%, letzte ca. -50%), gilt sportartübergreifend
