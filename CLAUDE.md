...
athletes (id uuid PK, strava_athlete_id bigint UNIQUE, strava_access_token text,
          ...
          strava_best_5k_seconds int,  ← von Strava selbst ermittelte 5k-Bestzeit (best_efforts/pr_rank aus der
                                      -- Activity-Detail-Response), via saveStrava5kPrIfPresent() (strava.ts) bei
                                      -- jedem erstmaligen Detail-Fetch einer Laufaktivität aktualisiert, wenn
                                      -- pr_rank=1 für "5k" gesetzt ist; Priorität in buildCoachSystemPrompt():
                                      -- best_5k_seconds (manuell) > strava_best_5k_seconds > Riegel-Schätzung
          strava_best_5k_at timestamptz, ← Datum der Aktivität hinter strava_best_5k_seconds
          strava_best_5k_backfill_done bool DEFAULT false, ← true nach dem einmaligen backfillStrava5kPr()-
...
- [x] Von Strava selbst ermittelte 5k-Bestzeit als Pace-Referenz-Quelle: `saveStrava5kPrIfPresent(athleteId, bestEfforts, activityDate)` (`strava.ts`) liest das `best_efforts`-Array aus der Strava Activity-Detail-Response (`fetchActivityDetail()`, dort um das Feld erweitert) und schreibt `athletes.strava_best_5k_seconds`/`strava_best_5k_at`, sobald ein Eintrag mit `name: "5k"` und `pr_rank: 1` vorkommt (= athletenweite Strava-Bestzeit zum Abfragezeitpunkt, direkt aus GPS/Zeit-Daten — nicht aus einem beliebig langen Lauf hochgerechnet). Aufruf an beiden bestehenden `fetchActivityDetail()`-Stellen, die ohnehin schon `splits_metric` cache-first laden: `activityAnalysis.ts`s `analyzeActivity()` (automatischer Pfad nach jedem Sync) UND `ActivityDetail.tsx` (falls die Hintergrund-Analyse beim Seitenaufruf noch nicht gelaufen ist) — kein separater API-Call nötig, da die Detail-Response ohnehin schon geholt wird. Läuft nur beim *erstmaligen* Detail-Fetch einer Aktivität (cache-first über `splits_metric_json`); Aktivitäten, deren Splits schon vor diesem Feature gecacht wurden, werden dadurch NICHT automatisch erfasst — dafür sorgt `backfillStrava5kPr()` (nächster Punkt). `buildCoachSystemPrompt()` priorisiert `best_5k_seconds` (manuell im Profil) > `strava_best_5k_seconds` (Strava-PR) > `estimateBest5kFromActivities()`-Riegel-Schätzung (nur wenn beides fehlt) — `calculatePaceReference()`s `best5kSource`-Parameter (`'estimated' | 'strava'`, vorher ein reines Bool) kennzeichnet Zielpace/Schwellenpace im Prompt-Text entsprechend
- [x] `backfillStrava5kPr(athleteId)` (`strava.ts`): einmaliger Nachhol-Check für genau die Lücke oben — holt für die letzten 20 Läufe (`BACKFILL_RUN_LIMIT`) gezielt `fetchActivityDetail()` unabhängig vom `splits_metric_json`-Cache (ohne diesen zu verändern) und wertet `best_efforts` per `saveStrava5kPrIfPresent()` aus. Gate über `athletes.strava_best_5k_backfill_done` (athletenweit, nicht pro Aktivität wie `recovery_checked`/`stimulus_checked`, da einmaliger Vorgang) — wird erst nach erfolgreichem Durchlauf gesetzt, unabhängig davon ob ein PR gefunden wurde. Bei Fehlschlag (z. B. `getValidAccessToken()` wirft wegen abgelaufenem Refresh-Token) bleibt das Flag `false` und der nächste App-Start versucht es erneut — silent failure analog zu `syncPushSubscription()`. Getriggert fire-and-forget in `App.tsx`s Layout-`useEffect` direkt neben `syncPushSubscription()`, no-op sobald das Flag gesetzt ist
- [x] Strava-PR sichtbar im Profil (`Profile.tsx`, "5k Bestzeit"-Feld): ist `best_5k_seconds` leer, zeigt eine "Strava-PR erkannt: MM:SS (Datum) — übernehmen?"-Zeile den Wert aus `athlete.strava_best_5k_seconds`/`strava_best_5k_at` als Klick-Vorschlag (übernimmt nur ins Eingabefeld, speichert nicht automatisch — Auto-Save greift danach wie gewohnt). Gleiche Priorität wie in `buildCoachSystemPrompt()`: die Riegel-Schätzungs-Zeile ("Geschätzt aus deinen letzten Läufen") erscheint nur, wenn WEDER eine manuelle PB NOCH ein Strava-PR vorliegt — die zugehörige `estimateBest5kFromActivities()`-Query beim Laden von Profile.tsx läuft entsprechend auch nur in diesem Fall
